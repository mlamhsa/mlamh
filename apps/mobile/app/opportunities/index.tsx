    ...
    <FlatList
      ...
      ListHeaderComponent={<View style={{ direction: isRtl ? "rtl" : "ltr" }}>
        <View style={[styles.topBar, isRtl && styles.topBarRtl]}>
          ...
          <Pressable accessibilityRole="button" accessibilityLabel={isArabic ? "التنبيهات" : "Notifications"} onPress={() => router.push("/notifications")} style={({ pressed }) => [styles.notificationButton, pressed && styles.pressed]}><Bell size={20} strokeWidth={1.9} color={theme.text} />{unreadCount > 0 ? <View style={styles.notificationBadge}><Text style={styles.notificationBadgeText}>{Math.min(unreadCount, 99)}</Text></View> : null}</Pressable>
        </View>
        ...
        <View style={styles.searchBox}><Search size={18} strokeWidth={1.9} color={theme.muted} /><TextInput value={query} onChangeText={setQuery} placeholder={isArabic ? "ابحث عن فرصة أو جهة" : "Search opportunities or companies"} placeholderTextColor={theme.muted} style={[styles.searchInput, isArabic && styles.arabicText, { textAlign }]} autoCapitalize="none" returnKeyType="search" /></View>
...
function createStyles(theme: typeof darkTheme) { return StyleSheet.create({
  screen: { flex: 1, backgroundColor: theme.background }, content: { paddingHorizontal: 20, paddingTop: 14, paddingBottom: 30, gap: 12 },
  topBar: { marginTop: 4, flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between", gap: 18 }, topBarRtl: { flexDirection: "row-reverse" }, titleBlock: { flex: 1, gap: 5 }, brand: { color: theme.accent, fontSize: 14, lineHeight: 20, fontWeight: "800", letterSpacing: 1.2 }, pageTitle: { color: theme.text, fontSize: 28, lineHeight: 34, fontWeight: "800" }, pageSubtitle: { color: theme.muted, fontSize: 13, lineHeight: 20, maxWidth: 390 },
  ...
  searchBox: { marginTop: 10, minHeight: 52, borderWidth: 1, borderColor: theme.border, borderRadius: 16, backgroundColor: theme.surface, paddingHorizontal: 14, flexDirection: "row", alignItems: "center", gap: 10 }, searchInput: { flex: 1, color: theme.text, fontSize: 14, paddingVertical: 12 },
