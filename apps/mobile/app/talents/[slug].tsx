...
import { BriefcaseBusiness, ChevronLeft, ChevronRight, Images, ShieldCheck, X } from "lucide-react-native";
...
export default function TalentProfileScreen() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (!active) return;
        setItem(talent);
        setOpportunities((dashboard?.opportunities ?? []).filter((opportunity) => opportunity.status !== "archived"));
      } catch {
  ...
  const facts = [
    item.city,
    country,
    item.age != null ? (ar ? `${formatLatinNumber(item.age, locale)} سنة` : `${formatLatinNumber(item.age, locale)} yrs`) : null,
  ...
  const languages = displayValues(item.languages, locale);
  const dialects = displayValues(item.dialects, locale);
  const skills = displayValues(item.skills, locale);
  const details = [
  ...
  const openInvite = () => {
    ...
    if (opportunities.length === 0) {
      router.push("/publisher/opportunities/new");
      return;
    }
    setInviteOpen(true);
  };
  ...
  return <SafeAreaView style={s.screen} edges={["top", "bottom"]}>
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.content}>
      <View style={[s.topRow, rtl && s.rowRtl]}>
        <Pressable accessibilityRole="button" accessibilityLabel={ar ? "رجوع" : "Back"} onPress={() => router.back()} style={s.iconButton}><Back size={21} color={darkTheme.text} /></Pressable>
        <Text style={[s.brand, txt(rtl)]}>{ar ? "ملامح للأعمال" : "MLAMH FOR BUSINESS"}</Text>
      </View>

      <View style={s.heroCard}>
        <Pressable accessibilityRole="button" accessibilityLabel={ar ? "فتح الصورة" : "Open photo"} onPress={() => gallery.length && setGalleryIndex(0)} style={s.heroImageWrap}>
          {item.imageUrl ? <Image source={{ uri: item.imageUrl }} style={s.heroImage} resizeMode="cover" /> : <View style={s.imageFallback}><Text style={s.imageInitial}>{item.name.slice(0, 1).toUpperCase()}</Text></View>}
        ...
        <View style={[s.identity, { alignItems: rtl ? "flex-end" : "flex-start" }]}>
          <View style={[s.nameRow, rtl && s.rowRtl]}><Text accessibilityRole="header" style={[s.name, txt(rtl)]}>{item.name}</Text>{item.verified ? <View style={s.verified}><ShieldCheck size={13} color={darkTheme.accent} /><Text style={s.verifiedText}>{ar ? "معتمد" : "Verified"}</Text></View> : null}</View>
          <Text style={[s.role, txt(rtl)]}>{role}</Text>
          <View style={[s.factRow, rtl && s.rowRtl]}>{facts.map((fact) => <View key={fact} style={s.factChip}><Text style={[s.factText, txt(rtl)]}>{fact}</Text></View>)}</View>
        </View>
      </View>

      {gallery.length > 1 ? <Section title={ar ? "معرض الصور" : "Gallery"} rtl={rtl}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={[s.galleryRow, rtl && s.galleryRowRtl]}>
          {gallery.map((url, index) => <Pressable key={`${url}-${index}`} onPress={() => setGalleryIndex(index)} style={s.galleryPress}><Image source={{ uri: url }} style={s.galleryImage} resizeMode="cover" /></Pressable>)}
        </ScrollView>
      </Section> : null}
      ...
      <Pressable accessibilityRole="button" onPress={openInvite} style={s.primary}>
        <BriefcaseBusiness size={18} color={darkTheme.background} />
        <Text style={s.primaryText}>{opportunities.length > 0 ? (ar ? "دعوة إلى فرصة" : "Invite to an opportunity") : (ar ? "أنشئ فرصة أولًا" : "Create an opportunity first")}</Text>
      </Pressable>
      <Text style={[s.ctaHint, txt(rtl)]}>{opportunities.length > 0 ? (ar ? "اختر إحدى فرصك الحالية بدل إنشاء فرصة جديدة." : "Choose one of your existing opportunities.") : (ar ? "لا توجد لديك فرصة حالية يمكن ربط الدعوة بها." : "You don't have an opportunity to attach an invitation to yet.")}</Text>
    </ScrollView>

    <Modal visible={galleryIndex !== null} animationType="fade" presentationStyle="fullScreen" onRequestClose={() => setGalleryIndex(null)}>
      <SafeAreaView style={s.viewer} edges={["top", "bottom"]}>
        <View style={[s.viewerTop, rtl && s.rowRtl]}><Text style={s.viewerCount}>{galleryIndex !== null ? `${formatLatinNumber(galleryIndex + 1, locale)} / ${formatLatinNumber(gallery.length, locale)}` : ""}</Text><Pressable onPress={() => setGalleryIndex(null)} style={s.viewerClose}><X size={22} color={darkTheme.text} /></Pressable></View>
        <FlatList
          horizontal
          pagingEnabled
          data={gallery}
          ...
          onMomentumScrollEnd={(event) => {
            ...
            if (width > 0) setGalleryIndex(Math.round(event.nativeEvent.contentOffset.x / width));
          }}
          renderItem={({ item: url }) => <View style={s.viewerPage}><Image source={{ uri: url }} style={s.viewerImage} resizeMode="contain" /></View>}
    ...
    <Modal visible={inviteOpen} animationType="slide" transparent onRequestClose={() => setInviteOpen(false)}>
      <View style={s.sheetBackdrop}><Pressable style={StyleSheet.absoluteFill} onPress={() => setInviteOpen(false)} /><SafeAreaView style={s.sheet} edges={["bottom"]}>
        <View style={[s.sheetHeader, rtl && s.rowRtl]}><View style={s.flex}><Text style={[s.sheetTitle, txt(rtl)]}>{ar ? "اختر الفرصة" : "Choose opportunity"}</Text><Text style={[s.sheetSubtitle, txt(rtl)]}>{ar ? `اختر الفرصة التي تريد ربط ${item.name} بها.` : `Choose the opportunity you want to use for ${item.name}.`}</Text></View><Pressable onPress={() => setInviteOpen(false)} style={s.viewerClose}><X size={20} color={darkTheme.text} /></Pressable></View>
        <ScrollView contentContainerStyle={s.opportunityList}>{opportunities.map((opportunity) => <Pressable key={opportunity.id} onPress={() => { setInviteOpen(false); router.push(`/publisher/opportunities/${opportunity.id}`); }} style={s.opportunityRow}><View style={s.flex}><Text style={[s.opportunityTitle, txt(rtl)]}>{opportunity.title}</Text><Text style={[s.opportunityStatus, txt(rtl)]}>{opportunity.published ? (ar ? "منشورة" : "Published") : (ar ? "غير منشورة" : "Not published")}</Text></View><ChevronLeft size={18} color={darkTheme.muted} style={!rtl ? { transform: [{ scaleX: -1 }] } : undefined} /></Pressable>)}</ScrollView>
      </SafeAreaView></View>
    </Modal>
  </SafeAreaView>;
}
