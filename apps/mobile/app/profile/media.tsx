...
export default function TalentMediaScreen() {
  ...
  return <ScrollView style={styles.screen} contentInsetAdjustmentBehavior="automatic" contentContainerStyle={[styles.content, { direction: isRtl ? "rtl" : "ltr" }]}>
    <View style={styles.top}><Pressable accessibilityRole="button" accessibilityLabel={isArabic ? "رجوع" : "Back"} onPress={() => router.back()} hitSlop={8} style={styles.backButton}><Text style={styles.back}>{isRtl ? "›" : "‹"}</Text></Pressable><Text style={styles.brand}>{isArabic ? "ملامح" : "MLAMH"}</Text></View>
    ...
    <View style={styles.counterRow}><Text style={styles.sectionTitle}>{isArabic ? "المعرض" : "Gallery"}</Text><Text accessibilityLabel={isArabic ? `${gallery.length} من ${MAX_GALLERY} صورة` : `${gallery.length} of ${MAX_GALLERY} images`} style={styles.counter}>{gallery.length}/{MAX_GALLERY}</Text></View>
    {gallery.length ? <View style={styles.grid}>{gallery.map((uri, index) => {
      const isPrimary = primaryUrl === uri; const busy = busyUrl === uri;
      return <View key={`${uri}-${index}`} style={styles.imageCard}><View><Image accessibilityLabel={isArabic ? `صورة ${index + 1}${isPrimary ? "، الصورة الرئيسية" : ""}` : `Image ${index + 1}${isPrimary ? ", primary image" : ""}`} source={{ uri }} style={[styles.image, isPrimary && styles.imagePrimary]} resizeMode="cover" />{isPrimary ? <Text style={styles.primaryBadge}>{isArabic ? "الرئيسية" : "Primary"}</Text> : null}</View><View style={styles.imageActions}>{!isPrimary ? <Pressable accessibilityRole="button" accessibilityLabel={isArabic ? `تعيين الصورة ${index + 1} رئيسية` : `Set image ${index + 1} as primary`} accessibilityState={{ disabled: Boolean(busyUrl), busy }} disabled={Boolean(busyUrl)} onPress={() => void makePrimary(uri)} style={({ pressed }) => [styles.secondaryAction, pressed && styles.pressed]}><Text style={styles.secondaryActionText}>{busy ? (isArabic ? "جارٍ التحديث…" : "Updating…") : (isArabic ? "تعيين رئيسية" : "Set primary")}</Text></Pressable> : <View style={styles.primarySpacer} />}<Pressable accessibilityRole="button" accessibilityLabel={isArabic ? `حذف الصورة ${index + 1}` : `Delete image ${index + 1}`} accessibilityState={{ disabled: Boolean(busyUrl) }} disabled={Boolean(busyUrl)} onPress={() => confirmDelete(uri)} style={({ pressed }) => [styles.deleteAction, pressed && styles.pressed]}><Text style={styles.deleteText}>{isArabic ? "حذف" : "Delete"}</Text></Pressable></View></View>;
    ...
    {error ? <View style={styles.errorBox}><Text accessibilityRole="alert" accessibilityLiveRegion="polite" style={styles.error}>{error}</Text></View> : null}
    <Pressable accessibilityRole="button" accessibilityLabel={isArabic ? "إضافة صورة إلى المعرض" : "Add image to portfolio"} accessibilityState={{ disabled: uploading || gallery.length >= MAX_GALLERY, busy: uploading }} disabled={uploading || gallery.length >= MAX_GALLERY} onPress={() => void addImage()} style={({ pressed }) => [styles.addButton, (pressed || uploading || gallery.length >= MAX_GALLERY) && styles.disabled]}>{uploading ? <ActivityIndicator accessibilityLabel={isArabic ? "جارٍ رفع الصورة" : "Uploading image"} color={theme.background} /> : <Text style={styles.addButtonText}>{isArabic ? "إضافة صورة" : "Add image"}</Text>}</Pressable>
