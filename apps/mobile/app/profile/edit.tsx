...
export default function EditTalentProfileScreen() {
  ...
  return <KeyboardAvoidingView style={styles.screen} behavior={Platform.OS === "ios" ? "padding" : undefined}><ScrollView contentContainerStyle={[styles.content, { direction: isRtl ? "rtl" : "ltr" }]} keyboardShouldPersistTaps="handled" contentInsetAdjustmentBehavior="automatic">
    <View style={[styles.topRow, isRtl && styles.rowRtl]}><Pressable accessibilityRole="button" accessibilityLabel={isArabic ? "رجوع" : "Back"} onPress={() => router.back()} hitSlop={8} style={styles.backButton}><Text style={styles.back}>{isArabic ? "رجوع" : "Back"}</Text></Pressable><Text style={styles.brand}>{isArabic ? "ملامح" : "MLAMH"}</Text></View>
    ...
    {error ? <View style={styles.errorBox}><Text accessibilityRole="alert" accessibilityLiveRegion="polite" style={[styles.error, { textAlign: align }]}>{error}</Text></View> : null}
    <Pressable accessibilityRole="button" accessibilityLabel={isArabic ? "حفظ تعديلات الملف" : "Save profile changes"} accessibilityState={{ disabled: saving, busy: saving }} disabled={saving} onPress={() => void save()} style={({ pressed }) => [styles.saveButton, saving && styles.disabled, pressed && styles.pressed]}>{saving ? <ActivityIndicator accessibilityLabel={isArabic ? "جارٍ حفظ الملف" : "Saving profile"} color={theme.background} /> : <Text style={styles.saveText}>{isArabic ? "حفظ التعديلات" : "Save changes"}</Text>}</Pressable>
    <Pressable accessibilityRole="button" accessibilityLabel={isArabic ? "مراجعة جاهزية الملف" : "Review profile readiness"} accessibilityState={{ disabled: saving }} disabled={saving} onPress={() => router.push("/profile/review")} style={({ pressed }) => [styles.reviewButton, saving && styles.disabled, pressed && styles.pressed]}><Text style={styles.reviewText}>{isArabic ? "مراجعة الجاهزية" : "Review readiness"}</Text></Pressable>
...
function createStyles(theme: typeof darkTheme) { return StyleSheet.create({
  ...
  suggestionList: { borderWidth: 1, borderColor: theme.border, borderRadius: 14, backgroundColor: theme.surface, overflow: "hidden" }, suggestion: { minHeight: 48, flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 12, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: theme.border }, suggestionText: { color: theme.text, fontSize: 12, fontWeight: "700" }, suggestionCode: { color: theme.accent, fontSize: 9, fontWeight: "800" },
  twoCol: { flexDirection: "row", gap: 9 }, col: { flex: 1 }, options: { flexDirection: "row", flexWrap: "wrap", gap: 7 }, option: { minHeight: 44, borderWidth: 1, borderColor: theme.border, backgroundColor: theme.surface, borderRadius: 999, paddingHorizontal: 11, paddingVertical: 8, justifyContent: "center" }, optionCompact: { minHeight: 44, paddingHorizontal: 9, paddingVertical: 7 }, optionActive: { borderColor: theme.accent, backgroundColor: theme.chip }, optionText: { color: theme.muted, fontSize: 11, fontWeight: "600" }, optionTextActive: { color: theme.text },
  toggleRow: { minHeight: 48, flexDirection: "row", alignItems: "center", justifyContent: "space-between", borderBottomWidth: 1, borderBottomColor: theme.border }, toggleLabel: { color: theme.text, fontSize: 13 }, switchTrack: { backgroundColor: theme.grayMuted }, switchTrackActive: { backgroundColor: theme.accent }, switchThumb: { backgroundColor: theme.text },
