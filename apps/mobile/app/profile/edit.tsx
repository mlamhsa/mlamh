...
export default function EditTalentProfileScreen() {
  ...
  const [citySlug, setCitySlug] = useState<string | null>(null); const [gender, setGender] = useState<string | null>(null); const [dateOfBirth, setDateOfBirth] = useState(""); const [nationalitySlug, setNationalitySlug] = useState(""); const [availabilityStatus, setAvailabilityStatus] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setCitySlug(i.citySlug); setGender(i.gender); setDateOfBirth(i.dateOfBirth ?? ""); setNationalitySlug(i.nationalitySlug ?? i.nationality ?? ""); setAvailabilityStatus(i.availabilityStatus);
  ...
  return <KeyboardAvoidingView style={styles.screen} behavior={Platform.OS === "ios" ? "padding" : undefined}><ScrollView contentContainerStyle={[styles.content, { direction: isRtl ? "rtl" : "ltr" }]} keyboardShouldPersistTaps="handled" contentInsetAdjustmentBehavior="automatic">
    ...
    <Section title={isArabic ? "البيانات الأساسية" : "Core details"} styles={styles}>
      ...
      <Field label={isArabic ? "الجنسية" : "Nationality"} value={nationalitySlug} onChangeText={setNationalitySlug} placeholder={isArabic ? "مثال: saudi" : "Example: saudi"} styles={styles} align={align} />
