  ...
  return <ScrollView style={styles.screen} contentContainerStyle={[styles.content, { direction: isRtl ? "rtl" : "ltr" }]} contentInsetAdjustmentBehavior="automatic">
    ...
    <View style={styles.languageCard}>
      ...
      <View style={[styles.languageOptions, isRtl && styles.rowRtl]}>
        <Pressable accessibilityRole="button" accessibilityState={{ selected: locale === "ar" }} onPress={() => chooseLocale("ar")} style={[styles.languageOption, locale === "ar" && styles.languageOptionActive]}>
          <Text style={[styles.languageOptionText, locale === "ar" && styles.languageOptionTextActive]}>العربية</Text>
        </Pressable>
        <Pressable accessibilityRole="button" accessibilityState={{ selected: locale === "en" }} onPress={() => chooseLocale("en")} style={[styles.languageOption, locale === "en" && styles.languageOptionActive]}>
    ...
    <View style={styles.group}>{rows.map((row, index) => <Pressable key={row.title} style={[styles.row, index === rows.length - 1 && styles.rowLast, isRtl && styles.rowRtl]} onPress={row.action}>
      <View style={styles.rowText}><Text style={[styles.rowTitle, isRtl && styles.textRtl]}>{row.title}</Text><Text style={[styles.rowSubtitle, isRtl && styles.textRtl]}>{row.subtitle}</Text></View>
