...
function coreReady(profile: MobileTalentProfile) {
  return Boolean(profile.displayName?.trim() && profile.citySlug && profile.gender && profile.dateOfBirth);
}
...
export default function TalentProfileJourneyScreen() {
  ...
  const progress = approved || submitted ? 100 : reviewReady ? 90 : hasCore && hasPhoto ? 90 : hasCore ? 65 : hasPhoto ? 45 : 25;
  ...
  const reviewBody = approved
    ...
    : submitted
      ...
      : reviewReady
        ? (isArabic ? "الأساسيات والصورة جاهزة. راجع ملفك ثم أرسله للمراجعة." : "Your essentials and photo are ready. Review and submit your profile.")
        : (isArabic ? "تفتح هذه الخطوة بعد اكتمال البيانات الأساسية والصورة الرئيسية." : "This step is ready once your core details and primary photo are complete.");
  ...
  return <SafeAreaView style={styles.screen} edges={["top", "bottom"]}>
    ...
    >
      ...
      <Text accessibilityRole="header" style={[styles.title, isRtl && styles.textRtl]}>{approved ? (isArabic ? "ملفك جاهز للفرص" : "Your profile is opportunity-ready") : submitted ? (isArabic ? "ملفك وصل للمراجعة" : "Your profile is in review") : (isArabic ? "خلّ ملفك جاهز للفرص" : "Get your profile opportunity-ready")}</Text>
      <Text style={[styles.subtitle, isRtl && styles.textRtl]}>{approved ? (isArabic ? "استمر في تطوير معرضك وبياناتك، واستكشف الفرص المناسبة لك." : "Keep your portfolio fresh and explore opportunities that fit you.") : submitted ? (isArabic ? "أنجزت المطلوب حاليًا. لا تحتاج لإعادة الإرسال؛ سنحدث الحالة هنا عند اتخاذ قرار المراجعة." : "You’ve completed the current setup. No need to resubmit; this screen updates when review status changes.") : (isArabic ? "نمشي معك خطوة بخطوة. ركّز على الأساسيات أولًا، ثم الصور، وبعدها أرسل ملفك للمراجعة." : "We’ll guide you step by step: core details first, then your portfolio, then review submission.")}</Text>
      ...
      <JourneyStep index="4" state={reviewState} icon={submitted ? <Check size={17} color={theme.background} strokeWidth={3}/> : <FileCheck2 size={18} color={reviewReady ? theme.accent : theme.muted}/>} title={isArabic ? "الجاهزية والمراجعة" : "Readiness & review"} body={reviewBody} action={submitted ? (isArabic ? "عرض حالة الملف" : "View profile status") : reviewReady ? (isArabic ? "مراجعة الجاهزية" : "Review readiness") : undefined} onPress={submitted ? () => router.replace("/profile") : reviewReady ? () => router.push({ pathname: "/profile/review", params: { onboarding: "1" } }) : undefined} styles={styles} isRtl={isRtl}/>
