...
export default function NotificationsScreen() {
  ...
  async function openNotification(item: MobileNotification) {
    ...
    if (item.category === "message" && item.referenceId) {
      router.push(`/conversations/${item.referenceId}`);
      return;
    }
    ...
    if (item.category === "invitation" && item.referenceId) {
      router.push(`/opportunities/${item.referenceId}`);
    }
  }
        ...
        ListHeaderComponent={
          <View style={[styles.header, { direction: isRtlLocale(locale) ? "rtl" : "ltr" }]}>
            ...
            <View style={styles.titleRow}>
              <Text style={styles.title}>{locale === "ar" ? "الإشعارات" : "Notifications"}</Text>
              {unreadCount > 0 ? <Text style={styles.unreadBadge}>{unreadCount}</Text> : null}
            </View>
            <Text style={styles.subtitle}>
              {locale === "ar" ? "تابع تحديثات طلباتك ورسائلك من مكان واحد." : "Keep up with application and message updates in one place."}
            </Text>
          </View>
        }
        ListEmptyComponent={
          <View style={styles.emptyState}>
            <Text style={styles.emptyTitle}>{error ?? (locale === "ar" ? "لا توجد إشعارات جديدة." : "No notifications yet.")}</Text>
          </View>
        }
        renderItem={({ item }) => (
          <Pressable
            onPress={() => void openNotification(item)}
            style={({ pressed }) => [styles.card, !item.isRead && styles.unreadCard, pressed && styles.pressed]}
          >
...
function createStyles(theme: typeof lightTheme | typeof darkTheme) {
  return StyleSheet.create({
    ...
    content: { paddingHorizontal: 18, paddingTop: 64, paddingBottom: 40, gap: 12 },
    ...
    unreadBadge: { minWidth: 28, textAlign: "center", color: "#181818", backgroundColor: theme.accent, borderRadius: 14, overflow: "hidden", paddingHorizontal: 8, paddingVertical: 4, fontSize: 12, fontWeight: "700" },
    subtitle: { color: theme.muted, fontSize: 15, lineHeight: 24 },
    card: { borderRadius: 22, borderWidth: 1, borderColor: theme.border, backgroundColor: theme.surface, padding: 17, gap: 7 },
    unreadCard: { borderColor: theme.accent },
    pressed: { opacity: 0.7 },
    cardTopRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
    category: { color: theme.accent, fontSize: 11, fontWeight: "700", textTransform: "uppercase", letterSpacing: 1 },
    ...
    emptyTitle: { color: theme.text, fontSize: 17, textAlign: "center" },
  });
}
