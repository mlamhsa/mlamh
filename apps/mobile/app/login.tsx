      ...
      <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <View style={[styles.content, { direction: isRtl ? "rtl" : "ltr" }]}>
          <View style={[styles.topRow, isRtl && styles.topRowRtl]}>
            ...
            <View style={styles.brandLockup}><Text style={[styles.brandArabic, isArabic && styles.arabicText]}>ملامح</Text><Text style={styles.brandLatin}>M L A M H</Text></View>
            <View style={styles.headerSpacer} />
          </View>

          <View style={styles.header}>
            <Text style={[styles.eyebrow, isArabic && styles.arabicText, { textAlign }]}>{isArabic ? "مرحبًا بعودتك" : "WELCOME BACK"}</Text>
            <Text accessibilityRole="header" style={[styles.title, isArabic && styles.arabicText, { textAlign }]}>{isArabic ? "ادخل إلى ملامح" : "Sign in to MLAMH"}</Text>
          ...
          <View style={styles.formCard}>
            <View style={styles.fieldWrap}>
              ...
              <View style={styles.inputShell}><Mail size={18} color={theme.muted} strokeWidth={1.8} /><TextInput accessibilityLabel={isArabic ? "البريد الإلكتروني" : "Email"} autoCapitalize="none" autoComplete="email" keyboardType="email-address" returnKeyType="next" placeholder="name@example.com" placeholderTextColor={theme.muted} value={email} onChangeText={setEmail} style={[styles.input, { textAlign }]} /></View>
            </View>
            <View style={styles.fieldWrap}>
              <Text style={[styles.fieldLabel, isArabic && styles.arabicText, { textAlign }]}>{isArabic ? "كلمة المرور" : "Password"}</Text>
              <View style={styles.inputShell}><LockKeyhole size={18} color={theme.muted} strokeWidth={1.8} /><TextInput accessibilityLabel={isArabic ? "كلمة المرور" : "Password"} autoCapitalize="none" autoComplete="current-password" secureTextEntry returnKeyType="done" placeholder="••••••••" placeholderTextColor={theme.muted} value={password} onChangeText={setPassword} style={[styles.input, { textAlign }]} onSubmitEditing={() => void signIn()} /></View>
            </View>
          ...
          <View style={styles.dividerRow}><View style={styles.dividerLine}/><Text style={styles.dividerText}>{isArabic ? "أو" : "OR"}</Text><View style={styles.dividerLine}/></View>
...
function createStyles(theme: typeof darkTheme) { return StyleSheet.create({
  screen: { flex: 1, backgroundColor: theme.background }, scrollContent: { flexGrow: 1, justifyContent: "center", paddingVertical: 12 }, content: { width: "100%", maxWidth: 520, alignSelf: "center", paddingHorizontal: 22, paddingTop: 4, paddingBottom: 30, gap: 22 },
  topRow: { minHeight: 80, flexDirection: "row", alignItems: "center", justifyContent: "space-between" }, topRowRtl: { flexDirection: "row-reverse" }, iconButton: { width: 42, height: 42, borderRadius: 21, borderWidth: 1, borderColor: theme.border, alignItems: "center", justifyContent: "center", backgroundColor: theme.surface }, headerSpacer:{width:42}, brandLockup:{alignItems:"center"}, brandArabic:{color:theme.accent,fontSize:34,lineHeight:38,fontWeight:"500"}, brandLatin:{color:theme.accent,fontSize:8,fontWeight:"900",letterSpacing:3.5,marginTop:-2},
  header: { gap: 7 }, eyebrow:{color:theme.accent,fontSize:10,fontWeight:"900",letterSpacing:2.6}, title: { color: theme.text, fontSize: 34, lineHeight: 41, fontWeight: "500", letterSpacing:-0.5 }, subtitle: { color: theme.muted, fontSize: 14, lineHeight: 22, maxWidth: 430 },
  formCard:{borderWidth:1,borderColor:theme.border,borderRadius:20,backgroundColor:theme.surface,padding:16,gap:13}, fieldWrap:{gap:7}, fieldLabel: { color: theme.text, fontSize: 12, lineHeight: 18, fontWeight: "700" }, inputShell:{minHeight:54,borderWidth:1,borderColor:theme.border,borderRadius:14,backgroundColor:theme.background,flexDirection:"row",alignItems:"center",gap:10,paddingHorizontal:14}, input:{flex:1,color:theme.text,fontSize:15,paddingVertical:12}, linkButton: { minHeight: 38, justifyContent: "center" }, forgotLink: { color: theme.accent, fontSize: 11, fontWeight: "700" },
  errorBox: { borderWidth: 1, borderColor: "#C84F4F66", backgroundColor: "#C84F4F14", borderRadius: 12, padding: 12 }, error: { color: "#E59A9A", fontSize: 13, lineHeight: 19 }, primaryButton: { backgroundColor: theme.accent, borderRadius: 14, minHeight: 54, alignItems: "center", justifyContent: "center" }, primaryButtonText: { color: theme.background, fontSize: 15, fontWeight: "900" }, dividerRow:{flexDirection:"row",alignItems:"center",gap:10}, dividerLine:{flex:1,height:1,backgroundColor:theme.border}, dividerText:{color:theme.muted,fontSize:9,fontWeight:"800"}, secondaryButton:{minHeight:52,borderWidth:1,borderColor:theme.border,borderRadius:14,alignItems:"center",justifyContent:"center",backgroundColor:theme.surface}, signupLink: { color: theme.text, fontSize: 13, textAlign: "center", fontWeight: "700" }, footnote:{color:theme.muted,fontSize:8,fontWeight:"800",letterSpacing:2,textAlign:"center",marginTop:4}, disabled: { opacity: 0.4 }, pressed: { opacity: 0.8 }, arabicText: { letterSpacing: 0 },
}); }
