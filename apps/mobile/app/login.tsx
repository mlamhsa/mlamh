    ...
    <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.scrollContent} contentInsetAdjustmentBehavior="automatic">
      <View style={[styles.content, { direction: isRtl ? "rtl" : "ltr" }]}>
        ...
        <View style={styles.form}>
          ...
          <Pressable accessibilityRole="button" onPress={() => router.push("/forgot-password")}><Text style={[styles.forgotLink, { textAlign }]}>{isArabic ? "نسيت كلمة المرور؟" : "Forgot password?"}</Text></Pressable>
        ...
        <Pressable accessibilityRole="button" accessibilityLabel={isArabic ? "إنشاء حساب" : "Create an account"} onPress={() => router.push("/signup")}><Text style={[styles.signupLink, isArabic && styles.arabicText]}>{isArabic ? "جديد في ملامح؟ أنشئ حسابك" : "New to MLAMH? Create your account"}</Text></Pressable>
      </View>
    </ScrollView>
