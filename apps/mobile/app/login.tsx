  ...
  return <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.screen}>
    <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.scrollContent} contentInsetAdjustmentBehavior="automatic">
      <View style={[styles.content, { direction: isRtl ? "rtl" : "ltr" }]}>
        ...
        <Pressable accessibilityRole="button" accessibilityLabel={isArabic ? "إنشاء حساب موهبة" : "Create a talent account"} onPress={() => router.push("/signup")}><Text style={[styles.signupLink, isArabic && styles.arabicText]}>{isArabic ? "جديد في ملامح؟ أنشئ حسابك" : "New to MLAMH? Create your account"}</Text></Pressable>
      </View>
    </ScrollView>
