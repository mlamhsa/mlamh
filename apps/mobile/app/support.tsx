      ...
      <ScrollView contentContainerStyle={[styles.content, compact && styles.contentCompact]} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        {ticketNumber ? <View style={styles.successCard}><View style={styles.successIcon}><CheckCircle2 size={26} color="#49C991"/></View><Text style={[styles.successTitle, isRtl && styles.textRtl]}>{isArabic ? "تم إرسال طلبك" : "Your request was sent"}</Text><Text style={[styles.successBody, isRtl && styles.textRtl]}>{isArabic ? "احتفظ برقم التذكرة للرجوع إلى طلبك مع فريق الدعم." : "Keep the ticket number to reference your request with support."}</Text><View style={styles.ticketBox}><Text style={styles.ticketNumber}>{ticketNumber}</Text></View><Pressable onPress={reset} style={styles.secondaryButton}><Text style={styles.secondaryButtonText}>{isArabic ? "إرسال طلب آخر" : "Send another request"}</Text></Pressable></View> : <>
          ...
          <View style={styles.formCard}>
            ...
            <Field label={isArabic ? "البريد الإلكتروني" : "Email"} value={email} onChangeText={setEmail} keyboardType="email-address" autoCapitalize="none" ltr isRtl={isRtl}/>
...
function Field({ label, ltr, isRtl, ...props }: { label: string; ltr?: boolean; isRtl?: boolean } & React.ComponentProps<typeof TextInput>) {
  ...
  return <View style={stylesStatic.fieldWrap}><Text style={[stylesStatic.label, rtl && stylesStatic.labelRtl]}>{label}</Text><TextInput placeholderTextColor="#8F8F89" style={[stylesStatic.input, rtl && !ltr && stylesStatic.textRtl, ltr && stylesStatic.ltr, rtl && ltr && stylesStatic.ltrRtl]} {...props}/></View>;
}

const stylesStatic = StyleSheet.create({ fieldWrap:{gap:7},label:{color:"#F5F5F0",fontSize:12,fontWeight:"800"},labelRtl:{textAlign:"right",writingDirection:"rtl"},textRtl:{textAlign:"right",writingDirection:"rtl"},input:{minHeight:52,borderWidth:1,borderColor:"#FFFFFF18",borderRadius:14,color:"#F5F5F0",fontSize:14,paddingHorizontal:14,paddingVertical:11,backgroundColor:"#0C0C0C"},ltr:{writingDirection:"ltr"},ltrRtl:{textAlign:"right"} });
