...
export default function ConversationScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!result) {
        setError(isArabic ? "هذه المحادثة غير متاحة لهذا الحساب أو تعذر الوصول إليها الآن." : "This conversation is unavailable for this account or cannot be reached right now.");
      } else {
        setData(result);
  ...
  if (!data) return <View style={styles.centered}><Text accessibilityRole="alert" style={styles.error}>{error}</Text><Pressable accessibilityRole="button" accessibilityLabel={isArabic ? "إعادة تحميل المحادثة" : "Reload conversation"} style={styles.secondaryButton} onPress={() => void load()}><Text style={styles.secondaryButtonText}>{isArabic ? "إعادة المحاولة" : "Try again"}</Text></Pressable><Pressable accessibilityRole="button" accessibilityLabel={isArabic ? "رجوع" : "Back"} onPress={() => router.back()}><Text style={styles.back}>{isArabic ? "رجوع" : "Back"}</Text></Pressable></View>;
  ...
  return <KeyboardAvoidingView style={styles.screen} behavior={Platform.OS === "ios" ? "padding" : undefined} keyboardVerticalOffset={8}>
    <View style={[styles.header, { direction: isRtl ? "rtl" : "ltr" }]}>
      <Pressable accessibilityRole="button" accessibilityLabel={isArabic ? "رجوع" : "Back"} onPress={() => router.back()} hitSlop={12} style={styles.backButton}><Text style={[styles.backIcon, isRtl && styles.backIconRtl]}>‹</Text></Pressable>
      ...
      <Text accessibilityLabel={liveUpdates ? (isArabic ? "التحديث المباشر متصل" : "Live updates connected") : (isArabic ? "جاري مزامنة المحادثة" : "Conversation syncing")} style={[styles.liveText, !liveUpdates && styles.syncingText]}>{liveUpdates ? (isArabic ? "مباشر" : "Live") : (isArabic ? "مزامنة" : "Syncing")}</Text>
    </View>
...
function createStyles(theme: typeof darkTheme) {
  return StyleSheet.create({
    ...
    header: { paddingHorizontal: 14, paddingTop: 50, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: theme.border, backgroundColor: theme.background, flexDirection: "row", alignItems: "center", gap: 10 }, rowRtl: { flexDirection: "row-reverse" }, backButton: { width: 44, height: 44, borderRadius: 22, alignItems: "center", justifyContent: "center" }, backIcon: { color: theme.text, fontSize: 30, lineHeight: 34 }, backIconRtl: { transform: [{ rotate: "180deg" }] },
    headerIdentity: { flex: 1, flexDirection: "row", alignItems: "center", gap: 10 }, avatar: { width: 40, height: 40, borderRadius: 20, borderWidth: 1, borderColor: theme.border, backgroundColor: theme.surface, alignItems: "center", justifyContent: "center" }, avatarText: { color: theme.accent, fontSize: 17, fontWeight: "800" }, headerText: { flex: 1, gap: 2 }, partyName: { color: theme.text, fontSize: 15, fontWeight: "800" }, contextLine: { color: theme.muted, fontSize: 10 }, liveText: { color: theme.accent, fontSize: 9, fontWeight: "800" }, syncingText: { color: theme.muted },
    ...
    composer: { borderTopWidth: 1, borderTopColor: theme.border, paddingHorizontal: 12, paddingTop: 9, paddingBottom: Platform.OS === "ios" ? 20 : 12, gap: 6, backgroundColor: theme.background }, composerRow: { flexDirection: "row", alignItems: "flex-end", gap: 8 }, composerRowRtl: { flexDirection: "row-reverse" }, input: { flex: 1, maxHeight: 120, minHeight: 48, borderWidth: 1, borderColor: theme.border, backgroundColor: theme.surface, color: theme.text, borderRadius: 16, paddingHorizontal: 14, paddingVertical: 11, fontSize: 15 }, sendButton: { minWidth: 68, minHeight: 48, borderRadius: 14, backgroundColor: theme.accent, alignItems: "center", justifyContent: "center", paddingHorizontal: 12 }, sendDisabled: { opacity: 0.4 }, sendText: { color: theme.background, fontWeight: "900", fontSize: 12 },
