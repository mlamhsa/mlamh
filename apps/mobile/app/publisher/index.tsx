...
export default function PublisherDashboardScreen() {
  ...
  return <View style={styles.screen}><FlatList
    ...
    ListHeaderComponent={<>
      ...
      <View style={styles.sectionHeader}><Text style={styles.sectionTitle}>{locale === "ar" ? "فرصك" : "Your opportunities"}</Text><Pressable style={styles.createButton} onPress={() => router.push("/publisher/opportunities/new")} accessibilityRole="button"><Text style={styles.createButtonText}>{locale === "ar" ? "+ فرصة جديدة" : "+ New opportunity"}</Text></Pressable></View>
      {error ? <Text style={styles.error}>{error}</Text> : null}
    </>}
    ListEmptyComponent={!error ? <View style={styles.empty}><Text style={styles.emptyTitle}>{locale === "ar" ? "لا توجد فرص بعد." : "No opportunities yet."}</Text><Text style={styles.meta}>{locale === "ar" ? "ابدأ بإنشاء أول فرصة من التطبيق." : "Create your first opportunity from the app."}</Text></View> : null}
    renderItem={({ item }) => <OpportunityRow item={item} locale={locale} styles={styles} />}
    ListFooterComponent={<Pressable style={styles.signOut} onPress={() => void signOutMobile().then(() => router.replace("/"))} accessibilityRole="button"><Text style={styles.signOutText}>{locale === "ar" ? "تسجيل الخروج" : "Sign out"}</Text></Pressable>}
