...
export default function PublisherDashboardScreen() {
  ...
  return <View style={styles.screen}><FlatList
    ...
    ListFooterComponent={<View style={styles.footer}><Pressable accessibilityRole="button" style={styles.supportRow} onPress={() => router.push("/support")}><Text style={styles.supportText}>{isArabic ? "الدعم · الخصوصية · الشروط · الاسترداد · الشكاوى" : "Support · Privacy · Terms · Refunds · Complaints"}</Text><Text style={styles.supportArrow}>{isRtl ? "‹" : "›"}</Text></Pressable><Pressable style={styles.signOut} onPress={() => void signOutMobile().then(() => router.replace("/"))} accessibilityRole="button"><Text style={styles.signOutText}>{isArabic ? "تسجيل الخروج" : "Sign out"}</Text></Pressable></View>}
...
function createStyles(theme: typeof darkTheme) { return StyleSheet.create({
  ...
  footer: { marginTop: 22 }, supportRow: { minHeight: 52, flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 10, borderTopWidth: 1, borderBottomWidth: 1, borderColor: theme.border }, supportText: { flex: 1, color: theme.muted, fontSize: 10, lineHeight: 16 }, supportArrow: { color: theme.accent, fontSize: 20 }, signOut: { paddingVertical: 16, alignItems: "center" }, signOutText: { color: "#E59A9A", fontSize: 12, fontWeight: "800" }
}); }
