...
export default function NewPublisherOpportunityScreen() {
  ...
  return <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.screen}><ScrollView contentContainerStyle={[styles.content, { direction: isRtlLocale(locale) ? "rtl" : "ltr" }]} keyboardShouldPersistTaps="handled">
    ...
    <Field label={locale === "ar" ? "عنوان الفرصة" : "Opportunity title"}><TextInput value={title} onChangeText={setTitle} placeholder={locale === "ar" ? "مثال: ممثل لإعلان تجاري" : "Example: Actor for commercial campaign"} placeholderTextColor={theme.muted} style={styles.input} /></Field>
    ...
    <Field label={locale === "ar" ? "المدينة" : "City"}><TextInput value={city} onChangeText={setCity} placeholder={locale === "ar" ? "الرياض" : "Riyadh"} placeholderTextColor={theme.muted} style={styles.input} /></Field>
    ...
    {compensation !== "unpaid" ? <Field label={locale === "ar" ? "الميزانية / المقابل" : "Budget / compensation"}><TextInput value={budget} onChangeText={setBudget} placeholder={locale === "ar" ? "مثال: 1500 ريال" : "Example: 1500 SAR"} placeholderTextColor={theme.muted} style={styles.input} /></Field> : null}
