  ...
  return <SafeAreaView style={styles.screen} edges={["top"]}>
    <FlatList
      ...
      ListHeaderComponent={<View style={[styles.header, { direction: isRtl ? "rtl" : "ltr" }]}>
        ...
        <View style={[styles.titleRow, isRtl && styles.rowRtl]}><View style={styles.titleIcon}><MessagesSquare size={20} color={theme.accent} strokeWidth={1.8} /></View><Text accessibilityRole="header" style={[styles.title, { textAlign: isRtl ? "right" : "left" }]}>{isArabic ? "الرسائل" : "Messages"}</Text>{unreadCount > 0 ? <View accessibilityLabel={isArabic ? `${unreadCount} رسائل غير مقروءة` : `${unreadCount} unread messages`} style={styles.unreadPill}><Text style={styles.unreadPillText}>{unreadCount > 99 ? "99+" : unreadCount}</Text></View> : null}</View>
        <Text style={[styles.subtitle, { textAlign: isRtl ? "right" : "left" }]}>{isArabic ? "المحادثات تفتح بعد القبول فقط، لحماية بيانات الطرفين." : "Conversations unlock only after acceptance, keeping both parties protected."}</Text>
        <View style={[styles.privacyNote, isRtl && styles.rowRtl]}><LockKeyhole size={15} color={theme.accent} strokeWidth={1.8} /><Text style={[styles.privacyText, { textAlign: isRtl ? "right" : "left" }]}>{isArabic ? "التواصل وبيانات الاتصال تبقى خاصة قبل القبول." : "Contact details stay private until acceptance."}</Text></View>
      </View>}
...
function ConversationRow({ item, locale, isRtl, styles }: { item: MobileConversation; locale: "ar" | "en"; isRtl: boolean; styles: ReturnType<typeof createStyles> }) {
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel={`${item.partyName}. ${item.latestMessage ?? item.opportunityTitle ?? ""}`} onPress={() => router.push(`/conversations/${item.id}`)} style={({ pressed }) => [styles.row, item.unreadCount > 0 && styles.rowUnread, isRtl && styles.rowRtl, pressed && styles.pressed]}>
    ...
    <View style={styles.rowBody}>
      ...
      {item.opportunityTitle ? <View style={styles.opportunityPill}><Text numberOfLines={1} style={styles.opportunity}>{item.opportunityTitle}</Text></View> : null}
