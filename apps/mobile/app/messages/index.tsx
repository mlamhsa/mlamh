...
function ActivityMetric({ value, label, styles, compact = false }: { value: string; label: string; styles: ReturnType<typeof createStyles>; compact?: boolean }) { return <View style={styles.activityMetric}><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.75} style={[styles.activityValue, compact && styles.activityValueCompact]}>{value}</Text><Text numberOfLines={1} style={styles.activityLabel}>{label}</Text></View>; }
function ConversationRow({ item, locale, isRtl, compact, styles }: { item: MobileConversation; locale: "ar" | "en"; isRtl: boolean; compact: boolean; styles: ReturnType<typeof createStyles> }) {
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel={`${item.partyName}. ${item.latestMessage ?? item.opportunityTitle ?? ""}`} onPress={() => router.push(`/conversations/${item.id}`)} style={({ pressed }) => [styles.row, compact && styles.rowCompact, item.unreadCount > 0 && styles.rowUnread, isRtl && styles.rowRtl, pressed && styles.pressed]}>
    ...
    <View style={styles.rowBody}>
      <View style={[styles.rowTop, isRtl && styles.rowRtl]}><Text numberOfLines={1} style={[styles.partyName, item.unreadCount > 0 && styles.partyNameUnread, { textAlign: isRtl ? "right" : "left", writingDirection: isRtl ? "rtl" : "ltr" }]}>{item.partyName}</Text><View style={[styles.timeRow, isRtl && styles.rowRtl]}><Clock3 size={11} color="#8F8F89" strokeWidth={1.8}/><Text style={styles.time}>{time}</Text></View></View>
...
function createStyles(theme: typeof darkTheme) { return StyleSheet.create({
  screen: { flex: 1, backgroundColor: theme.background }, content: { paddingHorizontal: 20, paddingTop: 20, paddingBottom: 118 }, contentCompact: { paddingHorizontal: 14, paddingTop: 14, paddingBottom: 106 },
  header: { gap: 10, marginBottom: 22 }, brand: { color: theme.accent, fontSize: 12, fontWeight: "900", letterSpacing: 1.9 }, arabicBrand: { letterSpacing: 0, writingDirection: "rtl" }, arabicText: { writingDirection: "rtl" }, titleRow: { flexDirection: "row", alignItems: "center", gap: 9 }, titleIcon: { width: 40, height: 40, borderRadius: 14, borderWidth: 1, borderColor: "#C9A96244", backgroundColor: "#C9A9620C", alignItems: "center", justifyContent: "center" }, title: { flexShrink: 1, color: theme.text, fontSize: 29, lineHeight: 35, fontWeight: "800" }, titleCompact: { fontSize: 26, lineHeight: 31 }, subtitle: { color: theme.muted, fontSize: 13, lineHeight: 20, maxWidth: 430 }, unreadPill: { minWidth: 25, height: 25, borderRadius: 13, backgroundColor: theme.accent, paddingHorizontal: 7, alignItems: "center", justifyContent: "center" }, unreadPillText: { color: theme.background, fontSize: 10, fontWeight: "900" }, activityStrip: { flexDirection: "row", gap: 8 }, activityMetric: { flex: 1, minHeight: 62, borderWidth: 1, borderColor: "#C9A96233", borderRadius: 16, backgroundColor: "#C9A96208", paddingHorizontal: 11, paddingVertical: 9, justifyContent: "center" }, activityValue: { color: theme.text, fontSize: 17, fontWeight: "900" }, activityValueCompact: { fontSize: 12 }, activityLabel: { marginTop: 2, color: theme.muted, fontSize: 9 }, privacyNote: { flexDirection: "row", alignItems: "center", gap: 8, borderWidth: 1, borderColor: "#C9A96233", borderRadius: 16, backgroundColor: "#C9A96208", paddingHorizontal: 13, paddingVertical: 11 }, privacyText: { flex: 1, color: theme.muted, fontSize: 10, lineHeight: 16 },
  row: { minHeight: 96, flexDirection: "row", alignItems: "center", gap: 12, borderWidth: 1, borderColor: theme.border, borderRadius: 20, backgroundColor: theme.surface, paddingHorizontal: 14, paddingVertical: 13, marginBottom: 10 }, rowCompact: { gap: 9, paddingHorizontal: 11, paddingVertical: 11 }, rowUnread: { borderColor: "#C9A96255", backgroundColor: "#C9A96208" }, rowRtl: { flexDirection: "row-reverse" }, pressed: { opacity: 0.72 }, avatar: { width: 52, height: 52, borderRadius: 26, borderWidth: 1, borderColor: theme.border, backgroundColor: theme.surface }, avatarCompact: { width: 46, height: 46, borderRadius: 23 }, avatarFallback: { alignItems: "center", justifyContent: "center" }, avatarUnread: { borderColor: theme.accent }, avatarInitial: { color: theme.accent, fontSize: 18, fontWeight: "800" },
  rowBody: { flex: 1, gap: 5 }, rowTop: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", gap: 8 }, partyName: { flex: 1, color: theme.text, fontSize: 15, fontWeight: "700" }, partyNameUnread: { fontWeight: "900" }, timeRow: { flexDirection: "row", alignItems: "center", gap: 4 }, time: { color: theme.muted, fontSize: 9 }, opportunityPill: { alignSelf: "flex-start", maxWidth: "100%", borderRadius: 999, backgroundColor: "#C9A9620C", paddingHorizontal: 8, paddingVertical: 3 }, opportunityPillRtl: { alignSelf: "flex-end" }, opportunity: { color: theme.accent, fontSize: 9, fontWeight: "800" }, previewRow: { flexDirection: "row", alignItems: "center", gap: 8 }, preview: { flex: 1, color: theme.muted, fontSize: 11, lineHeight: 17 }, previewUnread: { color: theme.text, fontWeight: "700" }, messageBadge: { minWidth: 20, height: 20, borderRadius: 10, backgroundColor: theme.accent, alignItems: "center", justifyContent: "center", paddingHorizontal: 5 }, messageBadgeText: { color: theme.background, fontSize: 8, fontWeight: "900" }, openIcon: { width: 32, height: 32, borderRadius: 16, borderWidth: 1, borderColor: theme.border, alignItems: "center", justifyContent: "center" },
