...
export function PublisherTabBar({ active, locale, theme, unreadCount = 0, notificationCount = 0 }: { active: PublisherTab; locale: AppLocale; theme: Theme; unreadCount?: number; notificationCount?: number }) {
  ...
  return <View style={styles.outer}><View style={styles.shell}>{tabs.map((tab) => {
    ...
    >
      <View style={[styles.iconWrap, selected && styles.iconWrapSelected, create && styles.createWrap]}>
        <Text accessible={false} style={[styles.glyph, selected && styles.glyphSelected, create && styles.createGlyph]}>{tab.glyph}</Text>
      ...
      <Text numberOfLines={1} style={[styles.label, selected && styles.selected, create && styles.createLabel]}>{label}</Text>
      {selected && !create ? <View importantForAccessibility="no-hide-descendants" style={styles.dot} /> : null}
    </Pressable>;
  })}</View></View>;
}
...
function createStyles(theme: Theme) {
  return StyleSheet.create({
    ...
    glyph: { color: theme.muted, fontSize: 18, lineHeight: 20, fontWeight: "600" },
    glyphSelected: { color: "#2E2E2E", fontWeight: "900" },
    createWrap: { width: 46, height: 46, borderRadius: 23, marginTop: -18, backgroundColor: theme.accent, borderWidth: 4, borderColor: theme.background },
    createGlyph: { color: "#2E2E2E", fontSize: 28, lineHeight: 30, fontWeight: "500" },
    label: { color: theme.muted, fontSize: 10, fontWeight: "700" },
    selected: { color: theme.accent },
    createLabel: { color: theme.accent, marginTop: 2 },
    dot: { width: 4, height: 4, borderRadius: 2, backgroundColor: theme.accent, marginTop: 1 },
    badge: { position: "absolute", top: -5, right: -11, minWidth: 17, height: 17, paddingHorizontal: 4, borderRadius: 9, alignItems: "center", justifyContent: "center", backgroundColor: theme.accent },
    badgeText: { color: "#2E2E2E", fontSize: 9, fontWeight: "900" },
  });
}
