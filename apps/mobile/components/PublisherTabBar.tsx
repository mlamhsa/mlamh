...
export function PublisherTabBar({ active, locale, theme = darkTheme, unreadCount = 0 }: { active: PublisherTab; locale: AppLocale; theme?: Theme; unreadCount?: number; notificationCount?: number }) {
  ...
  if (active === "talents" && viewer !== "publisher") {
    return <View style={[styles.outer, compact && styles.outerCompact, { paddingBottom: Math.max(insets.bottom, 8) }]}>
      <View style={[styles.publicShell, isArabic && styles.shellRtl, compact && styles.shellCompact]}>
        <Pressable accessibilityRole="button" onPress={() => router.replace("/opportunities")} style={({ pressed }) => [styles.publicAction, compact && styles.tabCompact, pressed && styles.pressed]}>
          <BriefcaseBusiness size={compact ? 17 : 18} strokeWidth={1.8} color={theme.muted} />
          <Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.82} style={[styles.publicActionText, compact && styles.labelCompact, isArabic && styles.arabicText]}>{isArabic ? "الفرص" : "Opportunities"}</Text>
        </Pressable>
        <View style={[styles.publicCurrent, compact && styles.tabCompact]}><Search size={compact ? 18 : 19} strokeWidth={2.1} color={theme.accent} /><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.82} style={[styles.publicCurrentText, compact && styles.labelCompact, isArabic && styles.arabicText]}>{isArabic ? "المواهب" : "Talents"}</Text></View>
        <Pressable accessibilityRole="button" onPress={() => router.push(viewer === "checking" ? "/login" : "/signup")} style={({ pressed }) => [styles.publicAction, compact && styles.tabCompact, pressed && styles.pressed]}>
          <UserRound size={compact ? 17 : 18} strokeWidth={1.8} color={theme.muted} />
          <Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.82} style={[styles.publicActionText, compact && styles.labelCompact, isArabic && styles.arabicText]}>{isArabic ? (viewer === "checking" ? "الدخول" : "انضم") : (viewer === "checking" ? "Sign in" : "Join")}</Text>
        </Pressable>
      </View>
    </View>;
  }
  ...
  return <View style={[styles.outer, compact && styles.outerCompact, { paddingBottom: Math.max(insets.bottom, 8) }]}>
    <View accessibilityRole="tablist" style={[styles.shell, isArabic && styles.shellRtl, compact && styles.shellCompact]}>
      {tabs.map((tab) => {
        ...
        const label = isArabic ? tab.ar : tab.en;
        const accessibilityLabel = badgeCount > 0 ? `${label}, ${badgeCount > 99 ? "99+" : badgeCount}` : label;
        const Icon = tab.icon;
        return <Pressable key={tab.key} accessibilityRole="tab" accessibilityLabel={accessibilityLabel} accessibilityState={{ selected }} hitSlop={5} style={({ pressed }) => [styles.tab, compact && styles.tabCompact, selected && styles.tabSelected, pressed && styles.pressed]} onPress={() => router.replace(tab.path)}>
          <View style={styles.iconWrap}>
            <Icon size={compact ? 17 : 18} strokeWidth={selected ? 2.15 : 1.8} color={selected ? theme.accent : theme.muted} />
            {badgeCount > 0 ? <View importantForAccessibility="no-hide-descendants" style={[styles.badge, isArabic && styles.badgeRtl]}><Text style={styles.badgeText}>{badgeCount > 99 ? "99+" : badgeCount}</Text></View> : null}
          </View>
          <Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.82} style={[styles.label, compact && styles.labelCompact, isArabic && styles.arabicText, selected && styles.labelSelected]}>{label}</Text>
        </Pressable>;
      })}
    </View>
  </View>;
...
function createStyles(theme: Theme) { return StyleSheet.create({
  ...
  outerCompact: { paddingHorizontal: 6 },
  ...
  shellCompact: { minHeight: 64, borderRadius: 21, paddingHorizontal: 3, paddingVertical: 5 },
  tab: { flex: 1, minHeight: 54, borderRadius: 17, alignItems: "center", justifyContent: "center", gap: 5, paddingHorizontal: 2 },
  tabCompact: { minHeight: 50, borderRadius: 15, gap: 3, paddingHorizontal: 1 },
  ...
  labelCompact: { fontSize: 8, lineHeight: 11 },
  ...
  badgeRtl: { right: undefined, left: -11 },
  ...
  publicActionText: { color: theme.muted, fontSize: 9, fontWeight: "700", textAlign: "center" },
  ...
  publicCurrentText: { color: theme.accent, fontSize: 9, fontWeight: "900", textAlign: "center" },
