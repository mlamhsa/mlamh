...
export function getMobileHrefFromUrl(rawUrl: string): Href | null {
  ...
  if (segments[0] === "talent" && segments[1]) {
    return `/talent/${encodeURIComponent(segments[1])}` as Href;
  }
  if (segments[0] === "talent") return "/talent" as Href;

  if (segments[0] === "opportunities") {
    if (segments[1] === "quick") return "/opportunities/quick" as Href;
    if (segments[1] === "casting") return "/opportunities/casting" as Href;
    if (segments[1]) {
  ...
  if (segments[0] === "scene") {
    ...
    if (segments[1] === "search") return "/scene/search" as Href;
    if (segments[1]) return `/scene/${encodeURIComponent(segments[1])}` as Href;
  ...
  if (segments[0] === "join") return "/join" as Href;
  if (segments[0] === "forgot-password") return "/forgot-password" as Href;
  if (segments[0] === "reset-password") return "/reset-password" as Href;
  if (segments[0] === "privacy") return "/legal/privacy" as Href;
  if (segments[0] === "terms") return "/legal/terms" as Href;
  if (segments[0] === "contact" || segments[0] === "complaints") {
