...
export function ManagedCastingScreen() {
  ...
  return <View style={styles.screen}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
    ...
    <View style={styles.steps}>{steps.map(([number,label]) => <View key={number} style={[styles.step,isArabic&&styles.rowReverse]}><Text style={styles.stepNo}>{number}</Text><Text style={[styles.stepLabel,{textAlign:align,writingDirection:direction}]}>{label}</Text></View>)}</View>
    ...
    <View style={styles.formCard}>
      ...
      <Field label={isArabic ? "البريد الإلكتروني" : "Email"} value={email} onChangeText={setEmail} isArabic={isArabic} forceLtr keyboardType="email-address" />
      <Field label={isArabic ? "رقم التواصل" : "Contact number"} value={phone} onChangeText={setPhone} isArabic={isArabic} forceLtr keyboardType="phone-pad" />
      ...
      <Field label={isArabic ? "تاريخ العمل الميلادي (YYYY-MM-DD)" : "Work date (YYYY-MM-DD)"} value={workDate} onChangeText={setWorkDate} isArabic={isArabic} forceLtr />
...
function Field({label,value,onChangeText,isArabic,forceLtr=false,keyboardType="default",placeholder}:{label:string;value:string;onChangeText:(v:string)=>void;isArabic:boolean;forceLtr?:boolean;keyboardType?:"default"|"email-address"|"phone-pad";placeholder?:string}) { const inputRtl=isArabic&&!forceLtr; return <View style={styles.field}><Text style={[styles.label,{textAlign:isArabic?"right":"left",writingDirection:isArabic?"rtl":"ltr"}]}>{label}</Text><TextInput value={value} onChangeText={onChangeText} keyboardType={keyboardType} autoCapitalize={keyboardType==="email-address"?"none":"sentences"} placeholder={placeholder} placeholderTextColor={colors.textMuted} style={[styles.input,{textAlign:inputRtl?"right":"left",writingDirection:inputRtl?"rtl":"ltr"}]} /></View>; }
