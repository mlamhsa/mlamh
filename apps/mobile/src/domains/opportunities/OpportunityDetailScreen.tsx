  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={["top"]}>
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {loading ? (
          <View style={styles.stateCard}><Text style={styles.stateText}>{isArabic ? "جارٍ تحميل التفاصيل..." : "Loading details..."}</Text></View>
        ) : error || !item ? (
          ...
          <>
                ...
                <View style={[styles.modePill, item.postingMode === "quick" ? styles.quickPill : styles.castingPill]}>
                  ...
                  <Text style={[styles.modeText, item.postingMode === "quick" && styles.quickText]}>
                    {item.postingMode === "quick"
                      ? (isArabic ? "طلب الآن" : "Quick Request")
                      : (isArabic ? "كاستينغ" : "Casting")}
                  </Text>
                </View>
            ...
            <View style={styles.semanticCard}>
              <Text style={[styles.semanticEyebrow, { textAlign: align }]}>
                {item.postingMode === "quick"
                  ...
                  : (isArabic ? "فرصة كاستينغ" : "CASTING OPPORTUNITY")}
              </Text>
            ...
            <Text style={styles.ctaNote}>
              {isArabic
                ? "سيتم ربط الإجراء بحالة حسابك واعتماد ملفك في مرحلة Auth/Talent Workspace القادمة."
                : "This action will be wired to account state and profile approval in the upcoming Auth/Talent Workspace phase."}
            </Text>
          </>
        )}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  cta: { minHeight: 58, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: spacing.sm, backgroundColor: colors.gold, borderRadius: radius.lg, marginTop: spacing.xl },
  ctaText: { color: "#090909", fontSize: 15, fontWeight: "800" },
  ctaNote: { color: "rgba(255,255,255,0.28)", fontSize: 10, lineHeight: 17, textAlign: "center", marginTop: spacing.sm, paddingHorizontal: spacing.md },
