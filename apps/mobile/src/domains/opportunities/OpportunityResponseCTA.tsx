...
function talentGate(
  ...
  postingMode: MobilePublicOpportunity["postingMode"],
  isArabic: boolean,
): CtaState {
  if (approvalStatus === "approved") {
    return {
      ...
      note:
        postingMode === "quick"
          ? isArabic
            ? "سيبدأ طلب مرتبط بمحادثة داخل ملامح."
            : "This starts a request-linked conversation inside MLAMH."
          : isArabic
            ? "سيتم إرسال ملفك للجهة للمراجعة."
            : "Your profile will be submitted for publisher review.",
      enabled: true,
    };
  }

  // Canonical Native/Web product state. Keep legacy submitted/pending as a
  // compatibility fallback for older accounts while data is normalized.
  if (
    ...
    return {
      ...
      note: isArabic
        ? "يمكنك التقديم بعد اعتماد ملف الموهبة."
        : "You can respond after your talent profile is approved.",
      enabled: false,
    };
  }

  if (approvalStatus === "ready_to_submit") {
    return {
      label: isArabic ? "أرسل ملفك للمراجعة" : "Submit profile for review",
      note: isArabic
...
function errorMessage(code: string, isArabic: boolean) {
  const ar: Record<string, string> = {
    ...
    QUICK_CONVERSATION_FAILED:
      "تم تسجيل الاهتمام لكن تعذر فتح المحادثة. حاول مرة أخرى لاحقًا.",
  };
  const en: Record<string, string> = {
    ...
    QUICK_CONVERSATION_FAILED:
      "Your interest was recorded, but the conversation could not be opened. Please try again later.",
  };
  return (
    (isArabic ? ar : en)[code] ??
    (isArabic
...
export function OpportunityResponseCTA({ item, locale }: Props) {
  ...
  const cta = useMemo<CtaState>(() => {
    ...
    if (session.status === "guest" || session.status === "account_missing") {
      return {
        ...
        note: isArabic
          ? "التقديم وإبداء الاهتمام متاحان لحسابات المواهب المؤهلة."
          : "Applications and interest are available to eligible talent accounts.",
        enabled: true,
    ...
    return talentGate(
      session.account.approvalStatus,
      item.postingMode,
      isArabic,
    );
  ...
  async function handlePress() {
    ...
    try {
      ...
      if (result.postingMode === "quick" && result.conversationId) {
        router.push(`/messages/${result.conversationId}` as Href);
        return;
      }
      ...
      setFeedback(
        item.postingMode === "quick"
          ? isArabic
            ? "تم إرسال اهتمامك وفتح مسار الطلب داخل ملامح."
            : "Your interest was sent and the request workflow is now open in MLAMH."
          : isArabic
            ? "تم إرسال طلبك للجهة بنجاح."
  ...
  const DisabledIcon =
    session.status === "talent" && !cta.enabled ? Clock3 : LockKeyhole;
  ...
  return (
    <View style={styles.wrapper}>
      ...
      >
        {!cta.enabled ? (
          <DisabledIcon size={17} color={colors.textMuted} />
        ) : null}
        <Text
          style={[styles.ctaText, !cta.enabled && styles.ctaTextDisabled]}
        >
          {submitting
            ? isArabic
              ? "جارٍ الإرسال..."
        ...
        {cta.enabled && !submitting ? (
          <DirectionArrow size={18} color="#090909" />
        ) : null}
      </Pressable>
