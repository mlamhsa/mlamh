...
export function SceneArticleScreen() {
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={["top"]}>
      <View style={styles.header}><Pressable onPress={() => router.back()} style={styles.back}><BackIcon size={20} color={colors.textSecondary} /></Pressable><Text style={styles.headerLabel}>{isArabic ? "مشهد ملامح" : "MLAMH SCENE"}</Text></View>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {!loading && !error && article ? <>
          ...
          <Text style={[styles.title, { textAlign: align }]}>{article.title}</Text>
          {article.excerpt ? <Text style={[styles.excerpt, { textAlign: align }]}>{article.excerpt}</Text> : null}
          ...
          <Text style={[styles.body, { textAlign: align }]}>{article.content}</Text>
          {article.tags.length ? <View style={styles.tags}>{article.tags.map((tag) => <View key={tag} style={styles.tag}><Text style={styles.tagText}>{tag}</Text></View>)}</View> : null}
        </> : null}
      </ScrollView>
    </SafeAreaView>
  );
