    ...
    <SafeAreaView style={styles.safeArea} edges={["top"]}>
      ...
      >
        ...
        <View style={[styles.searchBox, { flexDirection: isArabic ? "row-reverse" : "row" }]}>
          ...
          <TextInput value={query} onChangeText={setQuery} placeholder={isArabic ? "ابحث في مشهد ملامح..." : "Search MLAMH Scene..."} placeholderTextColor="rgba(255,255,255,0.28)" style={[styles.searchInput, { textAlign: align }]} />
        </View>
        ...
        {!loading && !error && data ? (
          <>
            ...
            {isSearch ? (
              <Section title={isArabic ? "نتائج البحث" : "Search results"}>
              ...
              <>
                ...
                {latest.length ? <Section title={isArabic ? "الأحدث" : "Latest"}>{latest.map((article) => <ArticleCard key={article.id} article={article} isArabic={isArabic} DirectionArrow={DirectionArrow} />)}</Section> : null}
                {data.reports.length ? <Section title={isArabic ? "تقارير" : "Reports"}>{data.reports.map((article) => <ArticleCard key={article.id} article={article} isArabic={isArabic} DirectionArrow={DirectionArrow} />)}</Section> : null}
                {data.stories.length ? <Section title={isArabic ? "قصص" : "Stories"}>{data.stories.map((article) => <ArticleCard key={article.id} article={article} isArabic={isArabic} DirectionArrow={DirectionArrow} />)}</Section> : null}
              </>
            )}
          </>
        ) : null}
...
function ArticleCard({ article, isArabic, DirectionArrow }: { article: SceneArticle; isArabic: boolean; DirectionArrow: typeof ChevronRight }) {
  return <Pressable onPress={() => router.push(`/scene/article/${article.slug}` as never)} style={styles.articleCard}>
