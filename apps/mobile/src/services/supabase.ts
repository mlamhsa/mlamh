...
export async function consumeNativeAuthCallback(rawUrl: string) {
  if (Platform.OS === "web" || typeof rawUrl !== "string" || rawUrl.length > 4096) {
    return false;
  }
  ...
  const accessToken =
    fragment.get("access_token") ?? parsed.searchParams.get("access_token");
  const refreshToken =
    fragment.get("refresh_token") ?? parsed.searchParams.get("refresh_token");
