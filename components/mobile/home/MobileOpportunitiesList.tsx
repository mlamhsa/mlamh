...
function formatCompensation(
  ...
  if (compensationType === "unpaid") return isArabic ? "غير مدفوع" : "Unpaid";
  if (compensationType === "negotiable") return isArabic ? "حسب الاتفاق" : "Negotiable";

  const amount = Number(budget);
  if (!Number.isFinite(amount) || amount <= 0) return isArabic ? "غير محدد" : "Not specified";

  return `${new Intl.NumberFormat(isArabic ? "ar-SA-u-nu-latn" : "en-US").format(amount)} ${
    isArabic ? "ريال" : "SAR"
  }`;
}

export async function MobileOpportunitiesList({ locale }: { locale: Locale }) {
  ...
  const opportunities = (await getPublishedOpportunities()).slice(0, 4);

  if (opportunities.length === 0) {
    return (
      <div className="px-4">
        <div className="rounded-[1.5rem] border border-white/[0.08] bg-white/[0.025] px-5 py-8 text-center">
          <BriefcaseBusiness size={25} strokeWidth={1.5} className="mx-auto text-gold/70" />
          <h3 className="mt-4 text-base font-medium text-white">
            {isArabic ? "لا توجد طلبات أو فرص منشورة حاليًا" : "No requests or opportunities yet"}
          </h3>
          <p className="mx-auto mt-2 max-w-[17rem] text-xs leading-6 text-white/35">
            {isArabic ? "سيتم نشر طلبات وفرص جديدة هنا فور توفرها." : "New requests and opportunities will appear here as soon as they're available."}
          </p>
        </div>
      </div>
    );
    ...
    <div dir={isArabic ? "rtl" : "ltr"} className="relative">
      <div className="space-y-3 px-4">
        {opportunities.map((item, index) => {
          ...
          const compensation = formatCompensation(item.compensation_type, item.budget, isArabic);
          const opportunityType =
            opportunityTypeLabels[item.opportunity_type]?.[isArabic ? "ar" : "en"] ?? item.opportunity_type;
          const href = `/${locale}/opportunities/${item.slug}`;
              ...
              <div className="relative">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex min-w-0 flex-wrap items-center gap-2">
                    ...
                    >
                      {isQuick ? <Zap size={11} strokeWidth={1.7} /> : <BriefcaseBusiness size={11} strokeWidth={1.7} />}
                      {isQuick
                        ? isArabic
                          ? "طلب الآن"
                          : "Quick Request"
                ...
                <h3 className="mt-4 line-clamp-2 text-[17px] font-semibold leading-7 text-white">{item.title}</h3>

                <div className="mt-3 flex min-w-0 items-center gap-2 text-xs text-white/45">
                  <Building2 size={13} strokeWidth={1.6} className="shrink-0 text-gold/65" />
                ...
                <div className="mt-4 grid grid-cols-2 gap-2">
                  <div className="min-w-0 rounded-[1rem] border border-white/[0.06] bg-black/15 px-3 py-2.5">
                    <div className="flex items-center gap-1.5 text-[10px] text-white/30">
                      <MapPin size={11} strokeWidth={1.6} className="text-gold/60" />
                      <span>{isArabic ? "الموقع" : "Location"}</span>
                    </div>
                    <p className="mt-1 truncate text-xs font-medium text-white/65">{location}</p>
                  </div>

                  <div className="min-w-0 rounded-[1rem] border border-white/[0.06] bg-black/15 px-3 py-2.5">
                    <div className="flex items-center gap-1.5 text-[10px] text-white/30">
                      <Wallet size={11} strokeWidth={1.6} className="text-gold/60" />
                      <span>{isArabic ? "المقابل" : "Compensation"}</span>
                    </div>
                    <p className="mt-1 truncate text-xs font-medium text-white/65">{compensation}</p>
                  </div>
                </div>
