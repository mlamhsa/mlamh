...
const allowedShareChannels = new Set(["native", "copy_link", "whatsapp"]);

export default function OpportunityViewTracker({ opportunityId }: OpportunityViewTrackerProps) {
  ...
  useEffect(() => {
    ...
    const storageKey = `mlamh:opportunity-view:${opportunityId}`;
    try {
      ...
      // If sessionStorage is blocked, do not block view telemetry.
    }

    const params = new URLSearchParams(window.location.search);
    const source = params.get("mlamh_source") === "opportunity_share" ? "opportunity_share" : undefined;
    const rawChannel = params.get("mlamh_channel") ?? "";
    const channel = allowedShareChannels.has(rawChannel) ? rawChannel : undefined;

    void trackOpportunityViewAction({ opportunityId, source, channel });

    return () => { cancelled = true; };
  }, [opportunityId]);

  if (!managedByMlamh) return null;
  ...
  return (
    <div className="mx-auto mb-3 w-full max-w-7xl">
      <div className="inline-flex max-w-full items-center gap-2 rounded-full border border-emerald-300/25 bg-emerald-300/[0.07] px-3 py-2 text-[10px] text-emerald-200 sm:px-4 sm:text-xs">
        ...
        <span className="truncate">{isEnglish ? "Managed by MLAMH Casting" : "إدارة الكاستينغ بواسطة ملامح"}</span>
      </div>
