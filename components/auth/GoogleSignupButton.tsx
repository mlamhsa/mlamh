...
export function GoogleSignupButton({ locale, accountType, intent }: Props) {
  ...
  async function handleGoogleSignup() {
    ...
    try {
      ...
      if (error) {
        console.error("[GoogleSignupButton]", error);
        setErrorMessage(
          isRtl
  ...
  return (
    <div className="mt-4">
      ...
      >
        ...
        <span>{loading ? (isRtl ? "جارٍ التحويل إلى Google..." : "Redirecting to Google...") : (isRtl ? "المتابعة باستخدام Google" : "Continue with Google")}</span>
      </button>
    </div>
  );
}
