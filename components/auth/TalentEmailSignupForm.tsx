...
export function TalentEmailSignupForm({ locale }: Props) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (cleanName.length < 2 || cleanName.length > 100) {
      return setErrorMessage(isRtl ? "أدخل اسمًا صحيحًا." : "Enter a valid name.");
    }
    if (!/^\+[1-9]\d{7,14}$/.test(normalizedPhone)) {
      return setErrorMessage(isRtl ? "أدخل رقم جوال صحيحًا مع مفتاح الدولة." : "Enter a valid mobile number with country code.");
    }
    if (!nationality || !gender || !residenceCountry || !city || !talentType) {
      return setErrorMessage(isRtl ? "أكمل جميع الحقول المطلوبة للمتابعة." : "Complete all required fields to continue.");
    }
    ...
    try {
      ...
      // Supabase can intentionally obscure duplicate-signup responses when email
  ...
  return (
    <form onSubmit={(event) => void submit(event)} className="space-y-5">
      ...
      {existingAccount?.exists ? (
        <div role="alert" className="rounded-2xl border border-gold/30 bg-gold/[0.07] p-5">
          ...
          <div className="mt-4 flex flex-col gap-3 sm:flex-row">
            <Link
              href={`/${locale}/login`}
              className="inline-flex min-h-11 flex-1 items-center justify-center rounded-xl bg-gold px-4 text-sm font-medium text-black transition hover:bg-gold-soft"
            >
              {isRtl ? "تسجيل الدخول" : "Sign in"}
            </Link>
            <Link
              href={`/${locale}/forgot-password`}
              className="inline-flex min-h-11 flex-1 items-center justify-center rounded-xl border border-white/10 px-4 text-sm text-white/70 transition hover:border-gold/35 hover:text-gold"
            >
      ...
      {errorMessage ? (
        <div role="alert" className="rounded-2xl border border-red-400/20 bg-red-400/10 px-4 py-3 text-sm text-red-300">
          {errorMessage}
        </div>
      ) : null}
          ...
          <select value={phoneCountry} onChange={(e) => { setPhoneCountry(e.currentTarget.value); setPhone(""); }} className={inputClass}>
            {TALENT_SIGNUP_COUNTRIES.map((country) => (
              <option key={country.code} value={country.code} className="bg-black">
                {country.dialCode} {isRtl ? country.ar : country.en}
              </option>
            ))}
          </select>
      ...
      <div>
        <p className="mb-3 text-sm font-medium text-white/75">
          {isRtl ? "ظهور الملف" : "Profile visibility"}{requiredMark}
        </p>
      ...
      <label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-white/10 bg-black/20 p-4">
        ...
        <span className="text-xs leading-6 text-white/55">
          {isRtl
            ? "أؤكد أن البيانات التي قدمتها صحيحة، وأسمح لملامح بحفظ بياناتي والتواصل معي بخصوص الفرص والمشاريع المناسبة. ولا تُستخدم صوري أو فيديوهاتي في إعلانات تجارية خارج خدمات ملامح دون موافقة مناسبة."
