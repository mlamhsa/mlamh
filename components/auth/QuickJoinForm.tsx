...
function SubmitButton({ isRtl }: { isRtl: boolean }) {
  ...
  return (
    <button type="submit" disabled={pending} aria-disabled={pending} className="inline-flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-gold px-5 text-sm font-medium text-black transition hover:bg-[#e0bd73] disabled:cursor-not-allowed disabled:opacity-60">
      {pending ? <><span aria-hidden="true" className="h-4 w-4 animate-spin rounded-full border-2 border-black/25 border-t-black"/><span>{isRtl ? "جارٍ إنشاء الحساب..." : "Creating account..."}</span></> : <span>{isRtl ? "إنشاء الحساب" : "Create account"}</span>}
    </button>
  );
}
...
export function QuickJoinForm({ locale, accountType, action }: QuickJoinFormProps) {
  ...
  return (
    <form action={action} className="space-y-5">
      ...
      <input type="hidden" name="countryIso" value={selectedCountry.code}/>
      ...
      <div>
        ...
        <input id="join-email" name="email" type="email" required inputMode="email" autoComplete="email" autoCapitalize="none" autoCorrect="off" spellCheck={false} value={email} onChange={(event) => setEmail(event.currentTarget.value)} placeholder="name@example.com" dir="ltr" className={`${inputClass} text-left`}/>
      </div>
      ...
      <div>
        ...
        <div className="relative">
          <input id="join-password" name="password" type={showPassword ? "text" : "password"} required minLength={8} autoComplete="new-password" placeholder={isRtl ? "8 أحرف على الأقل" : "At least 8 characters"} className={`${inputClass} ${isRtl ? "pr-4 pl-14" : "pl-4 pr-14"}`}/>
      ...
      <div>
        ...
        <div className="relative">
          <input id="join-password-confirmation" name="passwordConfirmation" type={showPasswordConfirmation ? "text" : "password"} required minLength={8} autoComplete="new-password" placeholder={isRtl ? "أعد كتابة كلمة المرور" : "Enter your password again"} className={`${inputClass} ${isRtl ? "pr-4 pl-14" : "pl-4 pr-14"}`}/>
      ...
      <label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-white/10 bg-black/20 p-4">
        <input name="acceptTerms" type="checkbox" value="accepted" required className="mt-1 h-5 w-5 shrink-0 accent-[#c8a96a]"/>
