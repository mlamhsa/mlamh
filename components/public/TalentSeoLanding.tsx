...
export function TalentSeoLanding({
  ...
  return (
    <main
      dir={isRtl ? "rtl" : "ltr"}
      className="min-h-screen bg-background px-4 pb-20 pt-24 text-white sm:px-6 lg:px-8 lg:pt-32"
    >
      <div className="mx-auto max-w-7xl">
        <header className="mb-10 max-w-4xl">
          ...
          <Link
            href={`/${locale}/talent`}
            className="mt-6 inline-flex rounded-xl border border-gold/30 px-4 py-2.5 text-sm text-gold transition hover:bg-gold/10"
          >
        ...
        {talents.length > 0 ? (
          <section className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
            {talents.map((talent) => (
              <PublicTalentCard key={talent.id} talent={talent} locale={locale} />
            ))}
          </section>
        ) : (
          <section className="rounded-[2rem] border border-white/10 bg-white/[0.025] p-8 text-center text-sm text-white/50">
            {isRtl
              ? "لا توجد ملفات منشورة ضمن هذا التصنيف حاليًا."
              : "No published profiles are available in this directory yet."}
          </section>
        )}
      </div>
    </main>
