    ...
    <form action={createAdminLocalizedOpportunityFormAction} className="grid gap-6">
      ...
      <section className="rounded-[2rem] border border-white/10 bg-white/[0.025] p-6 md:p-8">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          <Choice active={sourceType === "mlamh"} title="ملامح" description="تنشر وتدار مباشرة بواسطة ملامح." onClick={() => changeSource("mlamh")} />
          <Choice active={sourceType === "client"} title="نيابة عن عميل" description="ملامح تدير الفرصة والطلبات نيابة عن العميل." onClick={() => changeSource("client")} />
        </div>
          ...
          <div className="mt-7 border-t border-white/10 pt-7">
            ...
            <div className="mt-6">
              ...
              <div className="mt-3 grid gap-3 md:grid-cols-3">
                <Choice active={publicSourceMode === "client_name"} title="اسم العميل" description="يظهر اسم الجهة للموهبة." onClick={() => setPublicSourceMode("client_name")} />
                <Choice active={publicSourceMode === "mlamh_clients"} title="من عملاء ملامح" description="لا يظهر اسم العميل." onClick={() => setPublicSourceMode("mlamh_clients")} />
                <Choice active={publicSourceMode === "mlamh"} title="ملامح" description="تظهر كفرصة من ملامح." onClick={() => setPublicSourceMode("mlamh")} />
              </div>
            </div>
          </div>
        ) : null}
      ...
      <section className="rounded-[2rem] border border-[#c8a45d]/20 bg-white/[0.025] p-6 md:p-8">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h2 className="mt-1 text-xl font-semibold">محتوى الفرصة</h2>
            <p className="mt-2 text-sm leading-6 text-white/50">اكتب العنوان والوصف بالعربية فقط. ستنشئ ملامح النسخة الإنجليزية تلقائيًا عند الحفظ أو النشر.</p>
          </div>
      ...
      <div className="flex flex-wrap justify-end gap-3 pb-8">
        <button type="submit" name="publish_now" value="false" className="rounded-full border border-white/15 px-6 py-3 text-sm text-white/70 transition hover:bg-white/5">حفظ كمسودة</button>
        <button type="submit" name="publish_now" value="true" className="rounded-full bg-[#c8a45d] px-8 py-3 text-sm font-medium text-black transition hover:opacity-90">نشر الآن</button>
      </div>
    </form>
  );
}
