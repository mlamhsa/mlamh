...
export default function OpportunitySocialCreative({
  ...
  const [format, setFormat] =
    useState<CreativeFormat>("vertical");
...
const previewDimensions = {
  ...
  async function handleDownload() {
    ...
    try {
      ...
      link.download =
        `mlamh-opportunity-${slug}-${format}.png`;
      ...
      console.error(
        "[OpportunitySocialCreative.download]",
        error,
      );
    }
  }
    ...
    <section className="space-y-6">
      <div className="flex flex-wrap gap-3">
        <button
          ...
          className={[
            ...
            format === "vertical"
              ...
              : "border-white/10 text-white/55 hover:border-gold/30",
          ].join(" ")}
        >
          Story / Reel / TikTok
        </button>
        ...
        <button
          ...
          className="rounded-full border border-gold/40 px-5 py-3 text-sm text-gold transition hover:bg-gold hover:text-black"
        >
          تنزيل PNG
        </button>
      </div>
  ...
  <div className="relative grid h-full grid-rows-[280px_1fr_500px] px-[82px] py-[70px]">
    <header className="flex flex-col items-center justify-center">
      <img
        src="/brand/mlamh.svg"
        alt="MLAMH"
        className="h-auto w-[460px] object-contain"
      />

      <div className="mt-7 flex items-center gap-5">
        <div className="h-px w-[90px] bg-gradient-to-r from-transparent to-[#d2b06b]/50" />

        <p className="text-[22px] font-medium tracking-[0.08em] text-[#d2b06b]">
          فرصة جديدة
        </p>
    ...
    <main className="flex flex-col items-center justify-center text-center">
      <p className="mb-7 text-[21px] tracking-[0.16em] text-white/28">
        CASTING CALL
      </p>

      <h1 className="mx-auto max-w-[900px] text-[98px] font-light leading-[1.12] tracking-[-0.035em] text-white">
        {title}
      </h1>
      ...
      <div className="mt-14 flex items-center justify-center gap-7 text-[29px]">
        ...
        {city && opportunityType ? (
          <span className="text-[#d2b06b]/45">/</span>
        ) : null}
        ...
        {(city || opportunityType) && compensation ? (
          <span className="text-[#d2b06b]/45">/</span>
        ) : null}
      ...
      <p className="mt-9 text-[24px] text-white/35">
        تفاصيل الفرصة ومتطلبات التقديم على ملامح
      </p>
    </main>
    ...
    <footer className="flex items-end">
      <div className="w-full border-t border-white/[0.07] pt-12">
        <div className="grid grid-cols-[1fr_330px] items-center gap-[72px]">
          <div>
            <p className="text-[48px] font-light leading-[1.32] tracking-[-0.02em]">
              مهتم بالفرصة؟
              <br />
              قدّم مباشرة عبر ملامح
            </p>

            <p className="mt-7 max-w-[500px] text-[25px] leading-[1.75] text-white/38">
              امسح رمز QR لفتح صفحة الفرصة ومعرفة التفاصيل والتقديم.
            </p>

            <div className="mt-8 flex items-center gap-4">
              <span className="h-2 w-2 rounded-full bg-[#d2b06b]" />

              <span className="text-[25px] tracking-[0.08em] text-[#d2b06b]">
                mlamh.net
              </span>
            </div>
          </div>
