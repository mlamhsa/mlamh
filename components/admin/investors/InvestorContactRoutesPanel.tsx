...
export async function InvestorContactRoutesPanel({ isArabic }: { isArabic: boolean }) {
  ...
  const { data } = await db.from("investor_leads")
    .select("id,organization_name,organization_type,country_code,website_url,contact_email,contact_route_url,contact_route_type,contact_name,contact_role,status,fit_score")
    .eq("status", "qualified")
  ...
  return (
    <section className="mb-8 rounded-2xl border border-white/[0.08] bg-white/[0.02] p-5 sm:p-6" dir={isArabic ? "rtl" : "ltr"}>
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-xs leading-6 text-white/35">
            {isArabic
              ? "يعرض فقط القنوات العامة التي وجدها الوكيل من مصادر ويب فعلية. لا يتم تخمين أي بريد أو بيانات خاصة."
      ...
      <div className="grid gap-3 xl:grid-cols-2">
        {leads.map((lead) => {
          const officialRoute = lead.contact_route_url && !String(lead.contact_route_url).startsWith("mailto:")
            ? lead.contact_route_url
            : null;
          return (
            <article key={lead.id} className="rounded-xl border border-white/[0.07] bg-black/20 p-4">
              ...
              <div className="mt-3 text-[11px] leading-5 text-white/30">
                {lead.contact_email
