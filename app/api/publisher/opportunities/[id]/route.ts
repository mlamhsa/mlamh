...
import { updateMobilePublisherOpportunity } from "@/lib/publishers/mobile-opportunity-edit";
...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  let body: Record<string, unknown>;
  try { body = await request.json(); } catch { return NextResponse.json({ ok: false, code: "INVALID_BODY" }, { status: 400 }); }
  const result = await updateMobilePublisherOpportunity(auth.user.id, opportunityId, body);
  if (!result.ok) {
    const status = result.code === "NOT_FOUND" ? 404 : result.code.includes("PUBLISHER") ? 403 : result.code === "UPDATE_FAILED" ? 500 : 400;
    return NextResponse.json(result, { status });
  }
  return NextResponse.json(result);
}