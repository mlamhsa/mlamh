...
export async function GET() {
  ...
  const { data: publisherOpportunities, error: opportunitiesError } =
    await adminClient
      .from("opportunities")
      .select("id")
  ...
  if (opportunitiesError) {
    console.error("[PublisherDashboardCounts opportunities]", opportunitiesError);
  }

  const opportunityIds = (publisherOpportunities ?? []).map(
  ...
  if (applicantsError) {
    console.error("[PublisherDashboardCounts applicants]", applicantsError);
  }
  ...
  if (messagesResult.error) {
    console.error("[PublisherDashboardCounts messages]", messagesResult.error);
  }
  ...
  const unreadMessages = messagesResult.error
    ? 0
    : (messagesResult.data ?? []).length;

  const unreadNotifications = notificationsResult.error
    ? 0
    : ((notificationsResult.data ?? []) as NotificationRecord[]).filter(
