...
import { createMarketingOutreachAction, markLinkedInOutreachSentAction, sendApprovedOutreachNowAction } from "./actions";
    ...
    <AdminCard className="mb-5 overflow-hidden border-gold/15 bg-gradient-to-br from-gold/[0.055] via-white/[0.02] to-transparent">
      <div className="flex flex-col gap-4 p-5 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-sm leading-6 text-white/40">{isArabic ? "المسودة تدخل الاعتماد، ثم تُفتح صفحة الشخص لسوسن وترسل الرسالة يدويًا. بعد تعليمها كمرسلة ينشئ ملامح متابعة بعد 3 أيام تلقائيًا. بوابة التنفيذ الخارجي لا تتغير." : "The draft enters approval, then Sawsan opens the recipient profile and sends manually. Once marked sent, MLAMH automatically schedules a follow-up for three days later. The global execution gate is unchanged."}</p>
        </div>
    ...
    <AdminCard className="overflow-hidden">
      ...
      <div className="divide-y divide-white/[0.06]">
        {rows.length === 0 ? <div className="p-8 text-center text-sm text-white/40">{isArabic ? "لا توجد محاولات Outreach حقيقية بعد." : "No real outreach attempts yet."}</div> : rows.map((item) => {
          ...
          return <div key={item.id} className="group grid gap-4 px-5 py-4 transition-colors hover:bg-white/[0.018] lg:grid-cols-[minmax(0,1.5fr)_.6fr_.7fr_minmax(260px,1fr)] lg:items-center">
