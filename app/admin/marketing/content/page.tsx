...
export default async function ContentStudioPage({ searchParams }: PageProps) {
  ...
  return <AdminPageContainer>
    ...
    <AdminCard className="overflow-hidden"><div className="divide-y divide-white/[0.07]">{items.length === 0 ? <div className="p-6 text-sm text-white/40">{isArabic ? "لا يوجد محتوى حقيقي بعد." : "No real content records yet."}</div> : items.map((item) => <div key={item.id} className="grid gap-3 px-5 py-4 lg:grid-cols-[1.5fr_.7fr_.7fr_.7fr_1fr]"><div><div className="text-sm text-white">{item.title ?? item.hook ?? `#${item.id}`}</div><div className="mt-1 text-xs text-white/35">{item.objective ?? "—"}</div></div><div className="text-xs text-white/55">{item.content_type}</div><div className="text-xs text-white/55">{item.channel ?? "—"}</div><div className="text-xs text-gold">{item.status}</div><div>{["draft","review","ready"].includes(item.status) && item.channel ? <form action={requestContentPublishingApprovalAction}><input type="hidden" name="content_id" value={item.id}/><button className="rounded-xl border border-gold/25 bg-gold/10 px-3 py-2 text-xs text-gold">{isArabic ? "طلب اعتماد النشر" : "Request publishing approval"}</button></form> : <span className="text-xs text-white/35">{item.agent_id ?? "—"} · {item.language}</span>}</div></div>)}</div></AdminCard>
  </AdminPageContainer>;
}
