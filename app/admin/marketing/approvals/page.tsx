...
function ExternalReplyPreview({ action, isArabic, support }: { action: unknown; isArabic: boolean; support?: SupportContext }) {
  ...
  const recurring = /شهري|شهريا|مستمر|monthly|recurring/i.test(`${support?.subject ?? ""} ${support?.message ?? ""}`);
  const socialContent = /ريلز|reels|سوشيال|social|فيديو|video/i.test(`${support?.subject ?? ""} ${support?.message ?? ""}`);
  ...
  const executiveSummary = buildDanaExecutiveSummary({ language, talentType, talentCount: needed, city, recurring, socialContent, compensation: null, matchCount: matches.length, supplyMissing: missing });
  const drafts = buildDanaChannelDrafts({ language, senderName: text(recipient.name), talentType, city, matchNames, supplyMissing: missing });
  ...
  return (
    <div className="mt-5 space-y-4 rounded-2xl border border-gold/15 bg-black/20 p-4 sm:p-5">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        ...
        <div className="rounded-xl border border-white/[0.07] p-3"><p className="text-[10px] text-white/35">Email</p><p dir="ltr" className="mt-1 break-all text-sm text-white/75">{text(recipient.email) || "—"}</p></div>
        <div className="rounded-xl border border-white/[0.07] p-3"><p className="text-[10px] text-white/35">WhatsApp</p><p dir="ltr" className="mt-1 text-sm text-white/75">{text(recipient.phone) || "—"}</p></div>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-3">
        ...
        <div className="rounded-xl border border-white/[0.07] p-3"><p className="text-[10px] text-white/35">{isArabic ? "العجز" : "Supply gap"}</p><p className="mt-1 text-lg text-white/80">{missing}</p></div>
      </div>
      ...
      <form action={approveMarketingApproval} className="space-y-4 rounded-xl border border-gold/15 bg-gold/[0.025] p-4">
        ...
        <p className="text-xs text-gold">{isArabic ? "اختر قنوات الرد التي تريد تجهيزها" : "Choose reply channels to prepare"}</p>
        <div className="flex flex-wrap gap-4 text-xs text-white/70">
          <label className="flex items-center gap-2"><input type="checkbox" name="delivery_channels" value="email" defaultChecked={hasEmail} disabled={!hasEmail} /> Email</label>
          <label className="flex items-center gap-2"><input type="checkbox" name="delivery_channels" value="whatsapp" defaultChecked={hasPhone} disabled={!hasPhone} /> WhatsApp</label>
        </div>
        ...
        <p className="text-[10px] text-amber-200/70">{isArabic ? "الاعتماد ينشئ Jobs للقنوات المحددة فقط. external_execution_enabled يبقى هو حاجز التنفيذ ولا يحدث إرسال من هذه الشاشة." : "Approval only creates jobs for selected channels. external_execution_enabled remains the execution gate and this screen does not send anything."}</p>
        <button className="rounded-xl border border-gold/30 bg-gold/10 px-4 py-2 text-xs font-medium text-gold">{isArabic ? "اعتماد الرد وتجهيز القنوات" : "Approve reply & prepare channels"}</button>
      </form>
    </div>
  );
}
  ...
  return <AdminPageContainer>
    ...
    <div className="space-y-4">{approvals.length === 0 ? <AdminCard className="p-6 text-sm text-white/40">{isArabic ? "لا توجد طلبات اعتماد حاليًا." : "No approval requests."}</AdminCard> : approvals.map((item) => {
      ...
      return <AdminCard key={item.id} className="p-5">
        ...
        {externalReply ? <ExternalReplyPreview action={action} isArabic={isArabic} support={ticket ? supportContexts.get(ticket) : undefined} /> : null}
