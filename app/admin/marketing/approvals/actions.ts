...
async function applyExternalReplySideEffect({
  approvalId,
  task,
  proposedAction,
  decision,
  ...
  const idempotencyKey = `approval-${approvalId}-external-reply`;
  const now = new Date().toISOString();

  if (decision === "approved" || decision === "scheduled") {
    const channel =
      (typeof action.source_channel === "string" && action.source_channel.trim()) ||
      task.channel ||
      "support";
    ...
    if (error) throw new Error(`[external reply job] ${error.message}`);
    return;
  }
...
async function applyApprovalSideEffects({
  ...
  if (isExternalReply(proposedAction)) {
    await applyExternalReplySideEffect({
      approvalId,
      task,
      proposedAction,
      decision,
  ...
  if (task.task_type === "social_publish" && task.content_id) {
    if (decision === "approved" || decision === "scheduled") {
      ...
      if (channel === "buffer" && target !== "instagram" && target !== "facebook") {
        throw new Error("Buffer publishing target is required.");
      }
  ...
  if (task.task_type === "first_outreach") {
    ...
    if (Number.isInteger(outreachId) && outreachId > 0) {
      const sendStatus =
        decision === "approved" ? "approved" : decision === "scheduled" ? "scheduled" : "cancelled";
      await db
      ...
      if (channel === "email" && decision === "approved") {
        ...
        if (!recipientEmail || !subject || !text) {
          throw new Error("Approved email outreach is missing recipient, subject, or message.");
        }
        ...
        if (emailJobError && emailJobError.code !== "23505") {
          throw new Error(`[approval email channel job] ${emailJobError.message}`);
        }
      }
...
async function decideApproval(formData: FormData, decision: ApprovalDecision) {
  ...
  const { data: approval, error: readError } = await db
    ...
    .select("id,task_id,status,approval_level,channel,proposed_action")
    .eq("id", approvalId)
    .single();
  ...
  const executeAfter =
    decision === "scheduled" && executeAfterRaw ? new Date(executeAfterRaw).toISOString() : null;
...
export async function editMarketingApproval(formData: FormData) {
  ...
  if (readError || !approval || approval.status !== "pending") {
    throw new Error("Only pending approvals can be edited.");
  }

  const { error } = await db
    .from("marketing_approvals")
    .update({ reason, preview, updated_at: new Date().toISOString() })
    .eq("id", approvalId)
    .eq("status", "pending");
...
export async function approveMarketingApproval(formData: FormData) {
  await decideApproval(formData, "approved");
}

export async function rejectMarketingApproval(formData: FormData) {
  await decideApproval(formData, "rejected");
}

export async function cancelMarketingApproval(formData: FormData) {
  await decideApproval(formData, "cancelled");
}

export async function scheduleMarketingApproval(formData: FormData) {
  await decideApproval(formData, "scheduled");
}
