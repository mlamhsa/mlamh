...
type ManagedInvitationRow = { id: number; casting_role_id: number; opportunity_id: number; talent_id: number; status: string; sent_at: string | null; viewed_at: string | null; applied_at: string | null };
...
export default async function CastingApplicationsPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ lang?: string; role?: string }> }) {
  ...
  const [{ data: project }, { data: roles }, { data: shortlist }, { data: managedInvitations }] = await Promise.all([
    ...
    admin.from("managed_casting_invitations").select("id,casting_role_id,opportunity_id,talent_id,status,sent_at,viewed_at,applied_at").eq("casting_project_id", projectId),
  ]);
  if (!project) notFound();

  const roleRows = (roles ?? []) as CastingRoleRow[];
  const invitationRows = (managedInvitations ?? []) as ManagedInvitationRow[];
  ...
  const sourcingInvited = invitationRows.length;
  const sourcingViewed = invitationRows.filter((item) => ["viewed", "applied"].includes(item.status)).length;
  const sourcingApplied = invitationRows.filter((item) => item.status === "applied").length;

  return <div dir={isArabic ? "rtl" : "ltr"} className="px-4 py-6 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl">
    <div className="flex flex-col gap-4 border-b border-white/10 pb-6 lg:flex-row lg:items-end lg:justify-between"><div><Link href={`/admin/casting/${projectId}?lang=${language}`} className="text-xs text-gold hover:underline">{isArabic ? "← العودة للمشروع" : "← Back to project"}</Link><p className="mt-4 text-xs uppercase tracking-[0.28em] text-gold">MULTI-ROLE APPLICATIONS</p><h1 className="mt-2 text-3xl font-light text-white">{project.project_title}</h1><p className="mt-3 text-sm text-white/40">{isArabic ? "جميع طلبات فرص الأدوار في شاشة فرز واحدة، مع متابعة مسار الدعوات من المطابقة الداخلية حتى التقديم." : "All role-opportunity applications in one screening workspace, with managed sourcing invitation progress from match to application."}</p>{project.service_mode === "managed" ? <p className={`mt-3 text-xs ${canStartManagedConfirmation ? "text-emerald-300" : "text-amber-200"}`}>{canStartManagedConfirmation ? (isArabic ? "اعتمد العميل اختياراته — يمكنك بدء تأكيد المواهب المختارة." : "Client selections confirmed — talent confirmation can start.") : (isArabic ? "انتظار اعتماد العميل النهائي للاختيارات قبل بدء تأكيد المواهب." : "Waiting for the client's final selection approval before talent confirmation.")}</p> : null}</div>{project.service_mode === "managed" ? <Link href={`/admin/casting/${projectId}/supply?lang=${language}`} className="rounded-xl border border-gold/25 px-4 py-2.5 text-sm text-gold hover:bg-gold/10">{isArabic ? "المطابقة الداخلية" : "Internal supply"}</Link> : null}</div>

    {project.service_mode === "managed" ? <section className="mt-6 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl border border-white/10 bg-white/[0.025] p-4"><p className="text-xs text-white/35">{isArabic ? "الدعوات المرسلة" : "Invitations sent"}</p><p className="mt-2 text-2xl font-light text-white">{sourcingInvited}</p></div><div className="rounded-2xl border border-gold/15 bg-gold/[0.035] p-4"><p className="text-xs text-gold/65">{isArabic ? "تمت المشاهدة" : "Viewed"}</p><p className="mt-2 text-2xl font-light text-white">{sourcingViewed}</p></div><div className="rounded-2xl border border-emerald-300/15 bg-emerald-300/[0.035] p-4"><p className="text-xs text-emerald-200/65">{isArabic ? "تحولت إلى طلب" : "Converted to application"}</p><p className="mt-2 text-2xl font-light text-white">{sourcingApplied}</p></div></section> : null}
