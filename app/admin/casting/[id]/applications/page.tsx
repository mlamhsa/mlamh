...
type BookingRow = {
  ...
  status: string;
  publisher_completed_at: string | null;
  talent_completed_at: string | null;
};
...
export default async function CastingApplicationsPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ lang?: string; role?: string }> }) {
  ...
  const [{ data: talents }, { data: bookings }] = await Promise.all([
    talentIds.length
      ? admin.from("talents").select("id,name_ar,name_en,display_name_ar,display_name_en,slug,image_url,city_ar,city_en,category_ar,category_en").in("id", talentIds)
      : Promise.resolve({ data: [] as TalentRow[] }),
    applicationIds.length
      ? admin.from("talent_bookings").select("id,application_id,conversation_id,status,publisher_completed_at,talent_completed_at").in("application_id", applicationIds)
      : Promise.resolve({ data: [] as BookingRow[] }),
  ]);
  const talentRows = (talents ?? []) as TalentRow[];
  const shortlistRows = (shortlist ?? []) as ShortlistRow[];
  const bookingRows = (bookings ?? []) as BookingRow[];
  const talentMap = new Map<number, TalentRow>(talentRows.map((talent) => [Number(talent.id), talent]));
  const shortlistMap = new Map<number, ShortlistRow>(shortlistRows.map((item) => [Number(item.application_id), item]));
  const bookingMap = new Map<number, BookingRow>(bookingRows.map((item) => [Number(item.application_id), item]));
