...
export default async function ManagedCastingSupplyPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ lang?: string }> }) {
  ...
  const [{ data: project }, { data: roles }] = await Promise.all([
    ...
    admin.from("casting_roles").select("id,title,title_en,talent_type,required_count,requirements,status").eq("casting_project_id", projectId).neq("status", "cancelled").order("sort_order", { ascending: true }),
  ]);
  ...
  return <div dir={ar ? "rtl" : "ltr"} className="px-4 py-6 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl">
    <div className="flex flex-col gap-4 border-b border-white/10 pb-6 lg:flex-row lg:items-end lg:justify-between"><div><Link href={`/admin/casting/${projectId}?lang=${language}`} className="text-xs text-gold hover:underline">{ar ? "← العودة للمشروع" : "← Back to project"}</Link><p className="mt-4 text-xs uppercase tracking-[0.28em] text-gold">INTERNAL TALENT SUPPLY</p><h1 className="mt-2 text-3xl font-light text-white">{project.project_title}</h1><p className="mt-3 max-w-3xl text-sm leading-7 text-white/40">{ar ? "مطابقة داخلية لفريق ملامح فقط. تشمل المواهب العامة والخاصة المؤهلة ولا تنشئ طلبات أو Shortlist تلقائيًا." : "Internal MLAMH-only matching. Includes qualified public and private talent and never creates applications or shortlist entries automatically."}</p></div><Link href={`/admin/casting/${projectId}/applications?lang=${language}`} className="rounded-xl border border-gold/25 px-4 py-2.5 text-sm text-gold">{ar ? "مركز الفرز" : "Screening"}</Link></div>

    <div className="mt-6 space-y-6">{supplies.map(({ role, city, gender, minAge, maxAge, supply, matches, gap }) => <section key={role.id} className="rounded-2xl border border-white/10 bg-white/[0.025] p-5 sm:p-6"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-xl font-light text-white">{ar ? role.title : role.title_en || role.title}</h2><span className="rounded-full border border-white/10 px-2.5 py-1 text-[10px] text-white/45">{role.talent_type}</span></div><p className="mt-2 text-xs text-white/35">{[city, gender && gender !== "any" ? gender : null, minAge !== null || maxAge !== null ? `${minAge ?? "—"}-${maxAge ?? "—"}` : null].filter(Boolean).join(" · ") || (ar ? "بدون قيود إضافية" : "No additional constraints")}</p></div><div className="grid grid-cols-3 gap-2 text-center"><div className="rounded-xl border border-white/[0.07] bg-black/20 px-4 py-3"><p className="text-[10px] text-white/30">{ar ? "مطلوب" : "Needed"}</p><p className="mt-1 text-xl text-white">{gap.needed}</p></div><div className="rounded-xl border border-emerald-300/15 bg-emerald-300/[0.04] px-4 py-3"><p className="text-[10px] text-emerald-200/60">{ar ? "مطابق" : "Matches"}</p><p className="mt-1 text-xl text-white">{gap.available}</p></div><div className={`rounded-xl border px-4 py-3 ${gap.missing > 0 ? "border-amber-300/15 bg-amber-300/[0.04]" : "border-white/[0.07] bg-black/20"}`}><p className="text-[10px] text-white/30">Supply Gap</p><p className="mt-1 text-xl text-white">{gap.missing}</p></div></div></div>

      {matches.length === 0 ? <div className="mt-5 rounded-xl border border-amber-300/15 bg-amber-300/[0.04] p-4 text-sm text-amber-100/70">{ar ? "لا توجد مواهب مطابقة بالكامل حاليًا. راجع أسباب الاستبعاد أو وسّع شروط الدور." : "No fully sendable talent currently matches. Review exclusion reasons or widen the role criteria."}</div> : <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{matches.slice(0, 24).map((talent) => { const name = ar ? String(talent.display_name_ar || talent.name_ar || talent.display_name_en || talent.name_en || `Talent #${talent.id}`) : String(talent.display_name_en || talent.name_en || talent.display_name_ar || talent.name_ar || `Talent #${talent.id}`); const cityLabel = ar ? String(talent.city_ar || talent.city_en || "—") : String(talent.city_en || talent.city_ar || "—"); const image = typeof talent.image_url === "string" ? talent.image_url : null; const slug = typeof talent.slug === "string" ? talent.slug : null; const published = talent.published === true; return <article key={String(talent.id)} className="rounded-xl border border-white/[0.07] bg-black/20 p-4"><div className="flex items-center gap-3">{image ? <img src={image} alt="" className="h-14 w-14 rounded-xl object-cover"/> : <div className="h-14 w-14 rounded-xl border border-white/10"/>}<div className="min-w-0"><p className="truncate text-sm text-white/80">{name}</p><p className="mt-1 text-xs text-white/35">{cityLabel}</p><p className="mt-1 text-[10px] text-gold/60">{published ? (ar ? "ملف عام" : "Public profile") : (ar ? "ملف خاص — للمطابقة فقط" : "Private profile — matching only")}</p></div></div>{published && slug ? <Link href={`/${language}/talent/${slug}`} target="_blank" className="mt-3 inline-flex rounded-lg border border-white/10 px-3 py-2 text-xs text-white/55 hover:text-gold">{ar ? "عرض الملف" : "View profile"}</Link> : null}</article>; })}</div>}
