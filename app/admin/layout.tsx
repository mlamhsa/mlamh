...
async function getAdminSidebarCounts() {
  const adminClient =
    ...
    /*
     * الفرص التي تنتظر مراجعة الإدارة.
     */
    adminClient
      .from("opportunities")
      .select("id", {
  ...
  const {
    data: pendingPublisherProfiles,
    error: pendingPublisherProfilesError,
  } = await adminClient
    .from("profiles")
    .select("id")
  ...
  if (pendingOpportunitiesResult.error) {
    console.error(
      "[AdminLayout pendingOpportunities]",
  ...
  if (reportedMessagesResult.error) {
    console.error(
      "[AdminLayout reportedMessages]",
      reportedMessagesResult.error,
    );
  }

  if (
    unreadAdminNotificationsResult.error
  ) {
    console.error(
      "[AdminLayout unreadAdminNotifications]",
      unreadAdminNotificationsResult.error,
    );
  }
        ...
        >
          <AdminSidebar
            counts={{
              ...
              pendingPublishers:
                counts.pendingPublishers,

              pendingOpportunities:
                counts.pendingOpportunities,

              reportedMessages:
                counts.reportedMessages,

              notifications:
                counts.unreadAdminNotifications,
            }}
          />
        </Suspense>
