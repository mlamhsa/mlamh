...
export default function ClientAccountBanner({ locale, token, claimed, canClaim }: { locale: "ar" | "en"; token: string; claimed: boolean; canClaim: boolean }) {
  const ar = locale === "ar";
    ...
    <div className="mx-auto max-w-6xl rounded-2xl border border-white/10 bg-white/[0.025] p-4 sm:p-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="max-w-3xl">
          ...
          <p className="mt-2 text-sm text-white/65">{claimed
            ...
            : (ar ? "يمكنك متابعة المشروع بهذا الرابط بدون حساب، أو حفظه اختياريًا في حساب عميل عبر رابط دخول بالبريد — بدون كلمة مرور." : "You can keep using this private link without an account, or optionally save the project to a client account using a passwordless email link.")}</p>
          {feedback ? <p className="mt-2 text-xs leading-6 text-emerald-200">{feedback}</p> : null}
        </div>
        {claimed ? <Link href={`/${locale}/casting/client`} className="inline-flex min-h-11 shrink-0 items-center justify-center rounded-xl border border-gold/25 px-4 text-xs text-gold">{ar ? "مشاريعي" : "My projects"}</Link> : canClaim ? <form action="/api/casting/client/claim" method="post" className="shrink-0"><input type="hidden" name="token" value={token}/><input type="hidden" name="locale" value={locale}/><button className="min-h-11 rounded-xl border border-gold/25 bg-gold/[0.06] px-4 text-xs text-gold">{ar ? "احفظ المشروع في حسابي" : "Save project to my account"}</button></form> : null}
      </div>
    </div>
  </section>;
}
