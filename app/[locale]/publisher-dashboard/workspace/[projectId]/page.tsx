...
import {
  ...
  selectCastingApplicationAction,
...
export default async function WorkspaceProjectPage({ params }: PageProps) {
  ...
  if (!Number.isInteger(projectId) || projectId <= 0) redirect(`/${safeLocale}/publisher-dashboard/workspace`);
  ...
  if (!profile || profile.account_type !== "publisher") redirect(`/${safeLocale}/login`);

  const { data: publisher } = await admin
    .from("publishers")
    .select("id, publisher_type, status")
  ...
  const { data: project, error: projectError } = await admin
    ...
    .maybeSingle();
  if (projectError || !project) redirect(`/${safeLocale}/publisher-dashboard/workspace`);
  ...
  const shortlistRows = (shortlistData ?? []) as ShortlistRow[];
  ...
  const [{ data: applicantTalentsData }, { data: conversationsData }] = await Promise.all([
    talentIds.length
      ? admin.from("talents").select("id, slug, name_ar, name_en, image_url, city_ar, city_en").in("id", talentIds)
      : Promise.resolve({ data: [] }),
    applicationIds.length
      ? admin.from("conversations").select("id, application_id").in("application_id", applicationIds).eq("publisher_id", publisher.id)
      : Promise.resolve({ data: [] }),
  ]);

  const applicantTalentMap = new Map<number, TalentCard>((applicantTalentsData ?? []).map((talent) => [Number(talent.id), talent as TalentCard]));
  const shortlistByApplication = new Map<number, ShortlistRow>(shortlistRows.map((item) => [Number(item.application_id), item]));
  const conversationByApplication = new Map<number, number>((conversationsData ?? []).map((conversation) => [Number(conversation.application_id), Number(conversation.id)]));
    ...
    <div dir={isArabic ? "rtl" : "ltr"} className="space-y-6 pb-24 lg:pb-10">
      <header className="rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_top_right,rgba(201,169,98,0.13),transparent_38%),linear-gradient(135deg,rgba(255,255,255,0.04),rgba(255,255,255,0.01))] p-6 sm:p-8">
        ...
        <div className="mt-7 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-[10px] uppercase tracking-[0.3em] text-gold">SELF-SERVICE CASTING · ORGANIZATIONS</p>
        ...
        <div className="mt-7 grid grid-cols-2 gap-3 lg:grid-cols-5">
          ...
          <Stat label={isArabic ? "تم اختيارهم" : "Selected"} value={selectedCount} />
        </div>
      </header>

      <section className="space-y-4">
        {roleSupply.map(({ role, supply, gap }) => {
          const shortlistedForRole = shortlistRows.filter((item) => item.casting_role_id === role.id).length;
          const selectedForRole = shortlistRows.filter((item) => item.casting_role_id === role.id && item.status === "selected").length;
          ...
          return (
            <article key={role.id} className="rounded-[2rem] border border-white/10 bg-white/[0.025] p-5 sm:p-7">
              ...
              <div className="mt-5 grid grid-cols-4 gap-2">
                ...
                <Mini label={isArabic ? "مختار" : "Selected"} value={selectedForRole} />
              </div>
              ...
              <div className="mt-5 rounded-2xl border border-white/10 bg-black/20 p-4">
                {role.opportunity_id ? (
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <Link href={`/${safeLocale}/publisher-dashboard/opportunities/${role.opportunity_id}`} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-full border border-white/10 px-4 text-xs text-white/60 hover:border-gold/35 hover:text-gold"><ExternalLink size={14} />{isArabic ? "فتح الفرصة" : "Open opportunity"}</Link>
                  </div>
                ) : (
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <form action={publishCastingRoleAsOpportunityAction}>
                      <input type="hidden" name="locale" value={safeLocale} /><input type="hidden" name="projectId" value={project.id} /><input type="hidden" name="roleId" value={role.id} />
                ...
                <div className="mt-6">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    {roleApplications.slice(0, 8).map((application) => {
                      ...
                      const shortlistItem = shortlistByApplication.get(application.id);
                      const isShortlisted = Boolean(shortlistItem) || application.status === "shortlisted";
                      const isSelected = shortlistItem?.status === "selected" || application.status === "accepted";
                      const conversationId = conversationByApplication.get(application.id);

                      return (
                        <div key={application.id} className={`rounded-2xl border bg-black/20 p-4 ${isSelected ? "border-emerald-300/25" : "border-white/10"}`}>
                          <div className="flex items-center gap-3">
                            ...
                            {talent?.slug ? <Link href={`/${safeLocale}/talent/${talent.slug}`} className="text-[10px] text-gold">{isArabic ? "الملف" : "Profile"}</Link> : null}
                          </div>

                          {isSelected ? (
                            <div className="mt-3 space-y-2">
                              <div className="rounded-xl border border-emerald-300/20 bg-emerald-300/[0.06] px-3 py-2 text-center text-[10px] text-emerald-200">{isArabic ? "تم اختيار الموهبة" : "Talent selected"}</div>
                              {conversationId ? <Link href={`/${safeLocale}/booking/${conversationId}`} className="flex min-h-10 w-full items-center justify-center rounded-xl bg-gold px-4 text-xs font-medium text-black">{isArabic ? "متابعة الحجز" : "Continue booking"}</Link> : null}
                            </div>
                          ) : isShortlisted ? (
                            <div className="mt-3 space-y-2">
                              <div className="rounded-xl border border-gold/20 bg-gold/[0.06] px-3 py-2 text-center text-[10px] text-gold">{isArabic ? "ضمن القائمة المختصرة" : "Shortlisted"}</div>
                              <form action={selectCastingApplicationAction}>
                                <input type="hidden" name="locale" value={safeLocale} /><input type="hidden" name="projectId" value={project.id} /><input type="hidden" name="roleId" value={role.id} /><input type="hidden" name="applicationId" value={application.id} />
                                <button className="min-h-10 w-full rounded-xl bg-gold px-4 text-xs font-medium text-black">{isArabic ? "اختيار وبدء الحجز" : "Select & start booking"}</button>
                              </form>
                            </div>
                          ) : ["pending", "reviewing"].includes(String(application.status)) ? (
                    ...
                    {supply.sendableTalents.slice(0, 6).map((talent) => {
                      ...
                      return (
                        <div key={id} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-black/20 p-3">
                          ...
                          {slug && talent.published === true ? <Link href={`/${safeLocale}/talent/${slug}`} className="text-[10px] text-gold">{isArabic ? "الملف" : "Profile"}</Link> : <CheckCircle2 size={15} className="text-gold/60" />}
                        </div>
                      );
                    })}
                  </div>
      ...
      <details className="rounded-[2rem] border border-white/10 bg-white/[0.025] p-5 sm:p-7">
        ...
        <form action={addCastingRoleAction} className="mt-6 grid gap-4 sm:grid-cols-2">
          <input type="hidden" name="locale" value={safeLocale} /><input type="hidden" name="projectId" value={project.id} />
      ...
      <div className="rounded-2xl border border-white/10 bg-black/20 p-4 text-xs leading-6 text-white/40"><span className="inline-flex items-center gap-2 text-gold"><UsersRound size={14} />{isArabic ? "قاعدة مهمة" : "Important rule"}</span><p className="mt-2">{isArabic ? "نتائج المطابقة هي Supply فقط. لا تُضاف موهبة للقائمة المختصرة ولا يتم اختيارها إلا بعد تقديم فعلي على فرصة مرتبطة بالدور." : "Matching results are supply only. Talent enters the shortlist or selection only after a real application to the role's linked opportunity."}</p></div>
