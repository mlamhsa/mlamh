...
function formatWorkDuration(
  ...
  > = {
    ...
    "hour 1": { ar: "ساعة", en: "1 Hour" },
    "hours 2": { ar: "ساعتان", en: "2 Hours" },
    "hours 4": { ar: "4 ساعات", en: "4 Hours" },
  };
...
function opportunityStatusLabel(status: string | null, isRtl: boolean) {
  switch (status) {
    ...
    case "needs_changes":
      return isRtl ? "تحتاج تعديل" : "Needs Changes";
    default:
...
export default async function OpportunityDetailsPage({
  ...
  if (opportunity.status === "needs_changes") {
    const { data: reviewEvent, error: reviewEventError } = await adminClient
      .from("events")
      .select("metadata, created_at")
      .eq("event_type", "opportunity_needs_changes")
      .eq("target_id", String(publisher.id))
      .contains("metadata", { opportunityId: opportunity.id })
      .order("created_at", { ascending: false })
      .limit(1)
      .maybeSingle();

    if (reviewEventError) {
      console.error("Opportunity review event error:", reviewEventError);
    }
    ...
    reviewReason =
      typeof metadata?.reason === "string"
        ...
        : null;
    reviewRequestedAt = reviewEvent?.created_at ?? null;
  }
  ...
  const detailItems = [
    ...
    },
    {
      ...
      value: formatWorkDuration(opportunity.work_duration, locale),
    },
    {
      label: isRtl ? "عدد المواهب المطلوبة" : "Required Talents",
      value:
        opportunity.required_count !== null && opportunity.required_count !== undefined
    ...
    },
    {
      ...
      value: getFieldLabel(opportunity.required_gender, locale),
    },
  ];
  ...
  const roleRequirementItems: Array<{ label: string; value: string }> = [];

  if (normalizeKey(opportunity.opportunity_type) === "actor") {
    const languages = Array.isArray(roleRequirements.languages)
      ? roleRequirements.languages.map((item) => getFieldLabel(item, locale))
      : [];
    const dialects = Array.isArray(roleRequirements.dialects)
      ? roleRequirements.dialects.map((item) => getFieldLabel(item, locale))
      : [];
  ...
  if (normalizeKey(opportunity.opportunity_type) === "model") {
    const modelingTypes = Array.isArray(roleRequirements.modeling_types)
      ? roleRequirements.modeling_types.map((item) => getFieldLabel(item, locale))
      : [];
    ...
    if (formatValue(roleRequirements.hair_color)) {
      roleRequirementItems.push({
        ...
        value: getFieldLabel(roleRequirements.hair_color, locale),
      });
    }
  }
          ...
          <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
            <div className="max-w-3xl">
              <p className="text-xs text-amber-300">
                {isRtl ? "ملاحظات فريق ملامح" : "MLAMH Review"}
              </p>
              ...
              {reviewRequestedAt ? (
                <p className="mt-4 text-xs text-white/35">
                  {isRtl
                    ? `طلب التعديل: ${formatDate(reviewRequestedAt, locale)}`
                    : `Changes requested: ${formatDate(reviewRequestedAt, locale)}`}
                </p>
              ) : null}
            </div>
            <Link
