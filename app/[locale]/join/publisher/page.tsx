...
export default async function JoinPublisherPage({ params }: PageProps) {
  ...
  if (userError || !user) {
    redirect(`/${locale}/join?intent=publisher`);
  }

  const { data: profile, error: profileError } = await authClient
    .from("profiles")
    .select("id, account_type, display_name, onboarding_status, onboarding_step")
    .eq("user_id", user.id)
    .maybeSingle<ProfileRow>();

  if (profileError) {
    console.error("[JoinPublisherPage profileLookup]", profileError);
  }

  let hasPublisherRecord = false;
  if (profile?.account_type === "publisher") {
    const { data: publisherRecord, error: publisherRecordError } = await authClient
      .from("publishers")
      .select("id")
    ...
    if (publisherRecordError) {
      console.error("[JoinPublisherPage publisherLookup]", publisherRecordError);
    }
    ...
    >
      ...
      <div className="relative overflow-hidden pb-20 pt-28 md:pb-28 md:pt-32">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-10">
          <header className={`mb-10 sm:mb-14 md:mb-16 ${isRtl ? "text-right" : "text-left"}`}>
            <div className={`mb-6 flex items-center gap-4 ${isRtl ? "flex-row-reverse" : ""}`}>
              ...
              <p className="arabic-safe text-[10px] uppercase tracking-[0.4em] text-gold">
                {isRtl ? "تجهيز مساحة العمل" : "WORKSPACE SETUP"}
              </p>
            </div>
            ...
            >
              {isRtl ? "جهّز مساحة عملك" : "Set up your workspace"}
            </h1>
            ...
            >
              {isRtl
                ? "أخبرنا هل تبحث عن المواهب بصفتك الشخصية أو نيابة عن شركة أو جهة. بعد هذه الخطوة ستدخل مباشرة إلى مساحة العمل."
                : "Tell us whether you source talent personally or on behalf of an organization. After this step, you’ll go directly to your workspace."}
            </p>
          </header>

          <PublisherQuickSetupForm locale={locale} />
        </div>
