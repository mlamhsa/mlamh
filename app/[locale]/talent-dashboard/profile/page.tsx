...
export default function TalentProfileGuidedPage({
  ...
  const nationality = clean(talent?.nationality_slug) || clean(talent?.nationality) || (isArabic ? "غير محدد" : "Not set");
  ...
  function goToRequirement(key: string) {
    ...
    router.push(`/${locale}/talent-dashboard/profile/advanced#${
      key === "primary_role" ? "specialization" : key === "country" ? "residence-country" : key === "profile_visibility" || key === "data_accuracy_contact_consent" ? "privacy" : "identity"
    }`);
  }
      ...
      <div className="mx-auto max-w-5xl">
        <header className="mb-7">
          ...
          <div className="mt-3 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <h1 className="text-3xl font-light leading-tight sm:text-5xl">{isArabic ? "أكمل ملفك المهني" : "Complete your professional profile"}</h1>
              <p className="mt-3 max-w-2xl text-sm leading-7 text-white/50">{isArabic ? "جمعنا البيانات التي أدخلتها أثناء التسجيل. ركّز فقط على الخطوات المتبقية، وسنرشدك خطوة بخطوة." : "We've carried over the information from signup. Focus only on what's left and we'll guide you step by step."}</p>
            </div>
            <span className="inline-flex w-fit items-center rounded-full border border-emerald-400/20 bg-emerald-400/[0.06] px-4 py-2 text-xs text-emerald-200">{isArabic ? "✓ بيانات التسجيل محفوظة" : "✓ Signup data saved"}</span>
          </div>
        </header>
          ...
          <div className="grid gap-6 p-6 sm:p-8 lg:grid-cols-[1fr_280px] lg:items-center">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <span className="rounded-full border border-white/10 bg-black/25 px-3 py-1.5 text-xs text-white/65">{roleLabel}</span>
                <span className="rounded-full border border-white/10 bg-black/25 px-3 py-1.5 text-xs text-white/65">{city}</span>
                <span className="rounded-full border border-white/10 bg-black/25 px-3 py-1.5 text-xs text-white/65">{gender}</span>
                <span className="rounded-full border border-white/10 bg-black/25 px-3 py-1.5 text-xs text-white/65">{country}</span>
              </div>
              ...
              <p className="mt-3 text-sm leading-7 text-white/55">
                {readiness.isReady
                  ? isArabic ? "أكملت جميع متطلبات الاعتماد الأساسية." : "You've completed every core approval requirement."
                  : isArabic ? "لا نطلب منك إعادة البيانات التي سجلتها سابقًا. أكمل فقط العناصر الناقصة أدناه." : "We won't ask you to re-enter signup data. Complete only the missing items below."}
              </p>

              {!readiness.isReady && firstMissing ? (
                <button onClick={() => goToRequirement(firstMissing.key)} className="mt-6 inline-flex min-h-12 items-center justify-center rounded-full bg-gold px-7 text-sm font-semibold text-black transition hover:bg-[#e0bd73]">
            ...
            <div className="rounded-[1.5rem] border border-white/10 bg-black/25 p-5">
              <div className="flex items-end justify-between gap-4">
                ...
                <p className="text-xs text-white/45">{readiness.completedRequirements}/{readiness.totalRequirements}</p>
              </div>
              <div className="mt-5 h-2 overflow-hidden rounded-full bg-white/10">
                <div className="h-full rounded-full bg-gold transition-all" style={{ width: `${percentage}%` }} />
              </div>
              <p className="mt-4 text-xs leading-6 text-white/40">{isArabic ? "هذه النسبة تخص متطلبات الاعتماد فقط، وليست قوة ملفك." : "This score reflects approval requirements only, not profile strength."}</p>
            </div>
          </div>
        </section>
        ...
        <section className="mb-6 rounded-[2rem] border border-white/10 bg-white/[0.025] p-5 sm:p-7">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-[11px] uppercase tracking-[0.25em] text-gold">{isArabic ? "بياناتك الحالية" : "YOUR DETAILS"}</p>
              <h2 className="mt-2 text-2xl font-light">{isArabic ? "أدخلناها لك من التسجيل" : "Carried over from signup"}</h2>
            </div>
            <Link href={`/${locale}/talent-dashboard/profile/advanced#identity`} className="text-sm text-white/50 underline decoration-white/20 underline-offset-4 transition hover:text-gold">{isArabic ? "تعديل البيانات" : "Edit details"}</Link>
          </div>
        ...
        {!readiness.isReady ? (
          <section className="mb-6 rounded-[2rem] border border-white/10 bg-white/[0.025] p-5 sm:p-7">
            <div className="flex items-center justify-between gap-4">
              ...
              <span className="rounded-full border border-amber-400/20 bg-amber-400/[0.06] px-3 py-1.5 text-xs text-amber-200">{missingCount}</span>
            </div>

            <div className="mt-6 space-y-3">
              {readiness.missingRequirements.map((requirement, index) => (
                <button key={requirement.key} onClick={() => goToRequirement(requirement.key)} className="group flex w-full items-center justify-between gap-4 rounded-2xl border border-white/10 bg-black/20 p-4 text-start transition hover:border-gold/35 hover:bg-gold/[0.04]">
                  <div className="flex min-w-0 items-center gap-4">
                    <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-gold/25 bg-gold/[0.07] text-sm text-gold">{index + 1}</span>
                    <div>
                      <p className="text-sm text-white/85">{isArabic ? requirement.ar : requirement.en}</p>
                      <p className="mt-1 text-xs text-white/35">{requirement.key === "profile_image" ? (isArabic ? "صورة واضحة تمثلك مهنيًا" : "A clear professional photo") : requirement.key === "date_of_birth" ? (isArabic ? "بالتقويم الميلادي" : "Gregorian calendar") : (isArabic ? "مطلوب للاعتماد" : "Required for approval")}</p>
                    </div>
                  </div>
                  <span className="text-gold transition group-hover:translate-x-[-2px]" aria-hidden="true">{isArabic ? "←" : "→"}</span>
                </button>
              ))}
            </div>
          </section>
        ...
        {readiness.missingRequirements.some((item) => item.key === "date_of_birth") ? (
          <section className="mb-6 rounded-[2rem] border border-gold/20 bg-gold/[0.035] p-5 sm:p-7">
            ...
            <p className="mt-2 text-sm leading-7 text-white/50">{isArabic ? "نستخدم تاريخ الميلاد للمطابقة المناسبة والتحقق من أهلية بعض الفرص. أدخله بالتقويم الميلادي." : "We use your date of birth for matching and eligibility. Enter it using the Gregorian calendar."}</p>
            <div className="mt-5 flex flex-col gap-3 sm:flex-row">
              ...
              <button onClick={() => void saveBirthDate()} disabled={!birthDate || savingDate} className="min-h-14 rounded-2xl bg-gold px-7 text-sm font-semibold text-black disabled:cursor-not-allowed disabled:opacity-40">{savingDate ? (isArabic ? "جارٍ الحفظ..." : "Saving...") : (isArabic ? "حفظ والمتابعة" : "Save and continue")}</button>
            </div>
        ...
        <section className="grid gap-4 md:grid-cols-2">
          <div className="rounded-[2rem] border border-white/10 bg-white/[0.02] p-5 sm:p-6">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-[11px] uppercase tracking-[0.22em] text-white/35">{isArabic ? "بعد الاعتماد" : "AFTER APPROVAL"}</p>
                <h2 className="mt-2 text-xl font-light">{isArabic ? "قوِّ ملفك" : "Strengthen your profile"}</h2>
              </div>
              <span className="text-2xl font-light text-gold">{strength}%</span>
            </div>
            <p className="mt-3 text-sm leading-7 text-white/45">{isArabic ? "النبذة والمهارات والخبرة والمقاسات اختيارية للاعتماد، لكنها تساعدك في الظهور في ترشيحات أفضل." : "Bio, skills, experience and measurements are optional for approval, but help you rank for better matches."}</p>
            <Link href={`/${locale}/talent-dashboard/profile/advanced`} className="mt-5 inline-flex text-sm text-gold">{isArabic ? "تطوير الملف" : "Improve profile"} {isArabic ? "←" : "→"}</Link>
          </div>

          <div className="rounded-[2rem] border border-white/10 bg-white/[0.02] p-5 sm:p-6">
            <p className="text-[11px] uppercase tracking-[0.22em] text-white/35">{isArabic ? "معرض الأعمال" : "PORTFOLIO"}</p>
            <h2 className="mt-2 text-xl font-light">{isArabic ? "صورك وأعمالك في مكان واحد" : "Your work in one place"}</h2>
            <p className="mt-3 text-sm leading-7 text-white/45">{isArabic ? "أضف صورًا مهنية، رتّب معرضك واختر الصورة الرئيسية التي تظهر للجهات." : "Add professional work, organize your gallery and choose the main image publishers see."}</p>
            <Link href={`/${locale}/talent-dashboard/gallery`} className="mt-5 inline-flex text-sm text-gold">{isArabic ? "فتح معرض الأعمال" : "Open portfolio"} {isArabic ? "←" : "→"}</Link>
          </div>
        </section>
      </div>
    </main>
