...
export default function TalentCoreDetailsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  async function load(silent = false) {
    ...
    try {
      ...
      setNationality(normalizeNationalitySlug(clean(talent.nationality_slug) || clean(talent.nationality)));
        ...
        <form onSubmit={save} className="space-y-5">
          ...
          {showModelFields ? (
            <section className="rounded-[2rem] border border-gold/20 bg-gold/[0.03] p-5 sm:p-7">
              ...
              <h2 className="mt-2 text-xl font-light">{isArabic ? "حدد مجالات التصوير المناسبة لك" : "Choose the modeling work that fits you"}</h2>
              <p className="mt-2 text-sm leading-7 text-white/45">
                {isArabic
                  ? "ابدأ بتخصصات المودل لأنها تساعد ملامح والناشرين على مطابقتك مع الفرص المناسبة. يمكنك اختيار أكثر من تخصص."
                  : "Start with your modeling specializations so MLAMH and publishers can match you with relevant opportunities. You can choose more than one."}
              </p>

              <div className="mt-5 rounded-2xl border border-gold/20 bg-black/20 p-4 sm:p-5">
                <MultiChoiceField
                  label={isArabic ? "تخصصات المودل" : "Model specializations"}
                  values={modelingTypes}
                  onChange={setModelingTypes}
                  options={withLegacyValues(modelingTypes, MODEL_TYPE_CHOICES)}
                  isArabic={isArabic}
                  searchable
                  expanded
                />
                <p className="mt-3 text-xs leading-6 text-white/40">
                  {isArabic
                    ? "مثال: لتصوير الأحذية اختر «قدم — أحذية وإكسسوارات»، ولصالونات الشعر اختر «شعر — عناية وتصفيف»."
                    : "Example: choose “Foot — Shoes & Accessories” for footwear shoots or “Hair — Care & Styling” for salon work."}
                </p>
              </div>

              <h3 className="mt-7 text-base font-medium text-white/75">{isArabic ? "المقاسات والمظهر المهني" : "Professional measurements and appearance"}</h3>
...
function MultiChoiceField({ label, values, onChange, options, isArabic, searchable = false, expanded = false }: { label: string; values: string[]; onChange: (values: string[]) => void; options: TalentProfessionalChoice[]; isArabic: boolean; searchable?: boolean; expanded?: boolean }) {
