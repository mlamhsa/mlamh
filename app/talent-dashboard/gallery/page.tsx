...
export default async function TalentGalleryPage({ searchParams }: PageProps) {
  ...
  if (!talent) redirect("/ar/talent-dashboard");

  const gallery = normalizeGalleryImages(talent.gallery_images);
  const allImages = Array.from(new Set([talent.image_url, ...gallery].filter(Boolean) as string[]));
    ...
    <main className="min-h-screen bg-background px-6 py-10 text-white">
      <div className="mx-auto max-w-6xl">
        <header className="mb-10 flex flex-col gap-6 border-b border-white/[0.08] pb-8 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-[10px] uppercase tracking-[0.4em] text-gold">MLAMH TALENT</p>
            <h1 className="mt-3 text-4xl font-light tracking-tight text-white md:text-6xl" style={{ fontFamily: "var(--font-cormorant)" }}>My Gallery</h1>
            <p className="mt-3 text-sm text-gray-muted">Upload, reorder, and manage your gallery images.</p>
          </div>

          <div className="flex flex-wrap gap-3">
            <Link href="/talent-dashboard/profile" className="rounded-full border border-white/10 px-5 py-3 text-[10px] uppercase tracking-[0.3em] text-white/60 transition hover:border-gold/40 hover:text-gold">Edit Profile</Link>
            <Link href="/talent-dashboard" className="rounded-full border border-white/10 px-5 py-3 text-[10px] uppercase tracking-[0.3em] text-white/60 transition hover:border-gold/40 hover:text-gold">Back to Dashboard</Link>
          </div>
        </header>
    ...
    >
      Dismiss
        ...
        <section className="mb-8 rounded-3xl border border-white/[0.08] bg-gray-elevated/30 p-6">
          ...
          <form action={addOwnGalleryImageAction} className="mt-5 flex flex-col gap-3 md:flex-row">
            <input
              type="file"
              name="image_file"
              accept="image/*"
            ...
            />
            <button type="submit" className="rounded-xl border border-gold/40 bg-gold/[0.06] px-6 py-3 text-[10px] uppercase tracking-[0.25em] text-gold transition hover:bg-gold/10">Upload</button>
          </form>
        </section>
        ...
        {galleryData.length === 0 ? (
          ...
          <GallerySortableList
            images={galleryData.map((i) => i.url)}
