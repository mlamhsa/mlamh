import { TALENT_CATEGORIES } from "../data/talent-categories";
...
export type TalentQualificationInput = {
  ...
  profile_visibility?: "public" | "verified_publishers" | "private" | string | null;
};
...
export function evaluateTalentQualification(
  ...
  if (!text(talent.city_slug) && !text(talent.city_ar) && !text(talent.city_en)) reasons.push("missing_city");

  const qualified = reasons.length === 0;
  return { qualified, state: qualified ? "qualified" : "not_ready", reasons, image, role };
}
