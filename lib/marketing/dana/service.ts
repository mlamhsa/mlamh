...
async function acquireWorkflow(db: Db, input: CommercialInquiry, demandKey: string) {
  ...
  if (existingError || !existing) {
    throw new Error(`[Dana.acquireWorkflow] ${error?.message ?? existingError?.message ?? "workflow unavailable"}`);
  }
...
async function findExistingContact(db: Db, input: CommercialInquiry) {
  ...
  const [emailResult, phoneResult] = await Promise.all([
    email
      ? db.from("marketing_contacts").select("id,email,phone,metadata").ilike("email", email).order("id", { ascending: true }).limit(1).maybeSingle()
      : Promise.resolve({ data: null, error: null }),
    phone
      ? db.from("marketing_contacts").select("id,email,phone,metadata").eq("phone", phone).order("id", { ascending: true }).limit(1).maybeSingle()
      : Promise.resolve({ data: null, error: null }),
  ]);
  ...
  if (emailContact?.id && phoneContact?.id && Number(emailContact.id) !== Number(phoneContact.id)) {
    throw new Error("[Dana.resolveContact] email and phone resolve to different contacts; refusing unsafe merge");
  }
  return emailContact ?? phoneContact ?? null;
}

async function appendContactAliases(db: Db, contact: Record<string, unknown>, input: CommercialInquiry) {
  const email = normalizeEmail(input.senderEmail);
  const phone = normalizePhone(input.senderPhone);
  ...
  const patch: Record<string, unknown> = {
    metadata: {
      ...metadata,
      source_references: mergeSourceReferences(metadata.source_references, input.sourceReference),
    },
  };
  if (email && !contact.email) patch.email = email;
  if (phone && !contact.phone) patch.phone = phone;
  const { error } = await db.from("marketing_contacts").update(patch).eq("id", Number(contact.id));
...
async function persistResolvedWorkflowIdentity(
  ...
  const nextInput = {
    ...
    demand_key: demandKey,
    source_references: mergeSourceReferences(currentInput.source_references, input.sourceReference),
  };
  const { error } = await db.from("marketing_tasks").update({ input: nextInput }).eq("id", Number(task.id));
...
async function readTask(db: Db, taskId: number) {
  const { data, error } = await db.from("marketing_tasks").select("id,status,input,output").eq("id", taskId).maybeSingle();
  if (error || !data) throw new Error(`[Dana.waitWorkflow] ${error?.message ?? "workflow unavailable"}`);
...
async function waitForCompletedTask(db: Db, taskId: number) {
  for (let attempt = 0; attempt < WORKFLOW_WAIT_ATTEMPTS; attempt += 1) {
    ...
    if (task.status === "failed" || task.status === "cancelled") throw new Error(`[Dana.waitWorkflow] workflow ${task.status}`);
...
async function appendMetadataSourceReference(
  ...
  const { data, error } = await db.from(table).select("id,metadata").eq("id", entityId).maybeSingle();
  if (error || !data) throw new Error(`[Dana.aggregateSourceReference] ${table}: ${error?.message ?? "entity unavailable"}`);
  const metadata = (data.metadata ?? {}) as Record<string, unknown>;
  const sourceReferences = mergeSourceReferences(metadata.source_references, sourceReference);
  const { error: updateError } = await db.from(table).update({ metadata: { ...metadata, source_references: sourceReferences } }).eq("id", entityId);
  if (updateError) throw new Error(`[Dana.aggregateSourceReference] ${table}: ${updateError.message}`);
}

async function appendApprovalSourceReference(db: Db, approvalId: number, sourceReference: string) {
  const { data, error } = await db.from("marketing_approvals").select("id,proposed_action").eq("id", approvalId).maybeSingle();
  if (error || !data) throw new Error(`[Dana.aggregateSourceReference] approval: ${error?.message ?? "entity unavailable"}`);
  const proposedAction = (data.proposed_action ?? {}) as Record<string, unknown>;
  const sourceReferences = mergeSourceReferences(proposedAction.source_references, sourceReference);
  const { error: updateError } = await db.from("marketing_approvals").update({
    proposed_action: { ...proposedAction, source_references: sourceReferences },
  }).eq("id", approvalId);
  if (updateError) throw new Error(`[Dana.aggregateSourceReference] approval: ${updateError.message}`);
}

function preparedResultFromTask(task: TaskSnapshot, deduplicated: boolean): PreparedIntakeResult {
  const output = (task.output ?? {}) as Record<string, unknown>;
  const shortlist = (output.shortlist ?? {}) as Record<string, unknown>;
  const required = ["contact_id", "lead_id", "conversation_id", "brief_id", "draft_message_id", "approval_id"] as const;
  for (const key of required) {
    if (typeof output[key] !== "number") throw new Error(`[Dana.workflowResult] missing ${key}`);
  }
...
async function aggregateDuplicateSourceReference(db: Db, taskId: number, sourceReference: string) {
  for (let attempt = 0; attempt < WORKFLOW_WAIT_ATTEMPTS; attempt += 1) {
    ...
    if (!claimed) {
      ...
      if (current.status === "failed" || current.status === "cancelled") throw new Error(`[Dana.aggregateSourceReference] workflow ${current.status}`);
      await sleep(WORKFLOW_WAIT_MS);
      continue;
    }
    ...
    try {
      ...
      const sourceReferences = mergeSourceReferences(taskInput.source_references, sourceReference);
      await db.from("marketing_tasks").update({ input: { ...taskInput, source_references: sourceReferences } }).eq("id", taskId);
      await appendMetadataSourceReference(db, "marketing_contacts", result.contactId, sourceReference);
      await appendMetadataSourceReference(db, "marketing_leads", result.leadId, sourceReference);
      await appendMetadataSourceReference(db, "marketing_conversations", result.conversationId, sourceReference);
      await appendMetadataSourceReference(db, "marketing_briefs", result.briefId, sourceReference);
      await appendMetadataSourceReference(db, "marketing_messages", result.draftMessageId, sourceReference);
      await appendApprovalSourceReference(db, result.approvalId, sourceReference);
      await db.from("marketing_tasks").update({ status: "completed" }).eq("id", taskId);
      return { ...task, status: "completed", input: { ...taskInput, source_references: sourceReferences } } as TaskSnapshot;
...
async function returnDeduplicatedWorkflow(db: Db, task: TaskSnapshot, input: CommercialInquiry): Promise<PreparedIntakeResult> {
  const completed = task.status === "completed" ? task : await waitForCompletedTask(db, Number(task.id));
  const aggregated = await aggregateDuplicateSourceReference(db, Number(completed.id), input.sourceReference);
  const result = preparedResultFromTask(aggregated, true);
  await recordLifecycle(db, "deduplicated", result.demandKey, "marketing_task", String(result.taskId), {
    source_channel: input.sourceChannel,
    source_reference: input.sourceReference,
  });
  return result;
...
export async function processCommercialInquiry(input: CommercialInquiry): Promise<IntakeResult> {
  ...
  if (existingContact?.id) {
    await appendContactAliases(db, existingContact as Record<string, unknown>, input);
    const existingWorkflow = await findWorkflowByResolvedContact(db, input, classification, contactId);
    if (existingWorkflow) return returnDeduplicatedWorkflow(db, existingWorkflow, input);
  }
  ...
  if (!acquired.owner) return returnDeduplicatedWorkflow(db, acquired.task, input);

  const task = acquired.task;
  const taskId = Number(task.id);
  if (!contactId) contactId = await resolveContact(db, input);

  const demandKey = buildResolvedCommercialDemandKey(input, classification, contactId);
  await persistResolvedWorkflowIdentity(db, task, input, classification, contactId, demandKey);
  await recordLifecycle(db, "classified", demandKey, "marketing_task", String(taskId), {
    source_channel: input.sourceChannel,
    classification,
  });
  ...
  await recordLifecycle(db, "resolved", demandKey, "marketing_lead", String(leadId), {
    source_channel: input.sourceChannel,
    contact_id: contactId,
    conversation_id: conversationId,
  });
  ...
  await recordLifecycle(db, "brief", demandKey, "marketing_brief", String(briefId), {
    source_channel: input.sourceChannel,
  ...
  await recordLifecycle(db, "draft_prepared", demandKey, "marketing_message", String(draftMessageId), {
    source_channel: input.sourceChannel,
    external_execution: false,
  });
  await recordLifecycle(db, "approval_requested", demandKey, "marketing_approval", String(approvalId), {
    source_channel: input.sourceChannel,
  ...
  if (completionError) throw new Error(`[Dana.completeWorkflow] ${completionError.message}`);
