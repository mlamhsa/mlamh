...
test("expired promotional free aliases retry the same model family through free-only gateway providers", () => {
  assert.deepEqual(
...
test("lead enrichment fails closed when the gateway rejects a non-expired-free request", async () => {
