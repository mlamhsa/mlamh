...
/**
 * Creates an invitation signal using the existing events + notifications
 * infrastructure. It does not create an application on behalf of the talent;
 * the talent still chooses whether to open the opportunity and apply.
 */
export async function inviteTalentToPublisherOpportunity(input: {
  ...
  const { data: profile } = await supabase
    ...
    .eq("user_id", input.publisherUserId)
    .maybeSingle();
  if (!profile || profile.account_type !== "publisher" || profile.approval_status !== "approved" || isRestrictedAccountStatus(profile.status)) {
  ...
  const { data: opportunity } = await supabase
    .from("opportunities")
    .select("id,title,publisher_id,published,status")
  ...
  const { data: existing } = await supabase
    .from("events")
    .select("id")
    .eq("event_type", "talent_invited_to_opportunity")
    .eq("target_type", "opportunity")
    .eq("target_id", String(opportunity.id))
    .contains("metadata", { talent_id: Number(talent.id), publisher_id: Number(publisher.id) })
  ...
  const { data: event, error: eventError } = await supabase
    ...
    .single();
  if (eventError || !event) {
    console.error("[talent invitation event]", eventError);
    return { ok: false, code: "EVENT_FAILED" };
  }

  const title = input.locale === "ar" ? "دعوة إلى فرصة" : "Opportunity invitation";
  const body = input.locale === "ar"
    ? `تمت دعوتك للاطلاع على فرصة: ${opportunity.title}`
    : `You were invited to view the opportunity: ${opportunity.title}`;
  const { error: notificationError } = await supabase.from("notifications").insert({
    event_id: event.id,
    recipient_type: "talent",
    recipient_id: String(talent.id),
    title,
    body,
    is_read: false,
  });
  if (notificationError) {
    console.error("[talent invitation notification]", notificationError);
    return { ok: false, code: "NOTIFICATION_FAILED" };
  }
