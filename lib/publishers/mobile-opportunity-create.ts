...
export async function createMobilePublisherOpportunityDraft(userId: string, input: Record<string, unknown>) {
  ...
  const { data: publisher, error: publisherError } = await supabase
    .from("publishers")
    .select("id,company_name,contact_name,country_code,status")
  ...
  const countryCode = cleanText(input.countryCode, 2).toUpperCase() || publisher.country_code || "SA";
  const currency = cleanText(input.currency, 3).toUpperCase() || (countryCode === "SA" ? "SAR" : null);
  ...
  const { data: opportunity, error: insertError } = await supabase
    ...
    .select("id,title,status,published,country_code,created_at")
    .single();
