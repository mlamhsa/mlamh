...
async function createTalentNotification({
  ...
  const { data: event, error: eventError } = await admin
    .from("events")
    .insert({
      event_type: "managed_casting_booking_proposed",
      target_type: "booking",
  ...
  if (eventError || !event) {
    console.error("[managed casting booking event]", eventError);
    return;
  }
...
function revalidateManaged(projectId: number, conversationId?: number, token?: string | null) {
  ...
  revalidatePath("/ar/talent-dashboard/messages");
  revalidatePath("/en/talent-dashboard/messages");
...
export async function startManagedTalentConfirmationAction(formData: FormData) {
  ...
  const { data: project } = await admin
    ...
    .select("id,service_mode,client_access_token,client_selection_confirmed_at,work_date,city,currency,budget")
    .eq("id", projectId)
    .maybeSingle();
  ...
  const { data: application } = await admin
    ...
    .eq("id", shortlist.application_id)
    .maybeSingle();
  ...
  if (shortlist.casting_role_id) {
    const { data: role } = await admin
      ...
      .eq("id", shortlist.casting_role_id)
      .eq("casting_project_id", projectId)
      .maybeSingle();
  ...
  if (application.status !== "accepted") {
    ...
    await admin.from("application_status_logs").insert({
  ...
  if (!conversation) {
    ...
    const { error: assignmentError } = await admin
      .from("conversations")
      .update({ admin_user_id: adminUser.id, status: "active", updated_at: now })
      .eq("id", conversation.id);
    if (assignmentError) throw new Error(assignmentError.message);
  }

  const compensationType = ["fixed", "negotiable", "unpaid"].includes(String(opportunity.compensation_type))
    ? String(opportunity.compensation_type)
    : "negotiable";
  const compensationAmount = compensationType === "fixed"
    ? parseBudgetAmount(opportunity.budget ?? project.budget)
    : null;
  ...
  const payload = {
    ...
    publisher_id: null,
    admin_user_id: adminUser.id,
    managed_casting_project_id: projectId,
    talent_id: application.talent_id,
    status: "proposed",
  ...
  const saveQuery = existingBooking
    ? admin.from("talent_bookings").update(payload).eq("id", existingBooking.id).select("id").single()
  ...
  await createTalentNotification({
    ...
    opportunityId: Number(application.opportunity_id),
    title: "تم اختيارك لمشروع مُدار بواسطة ملامح",
    body: "راجع تفاصيل العمل وأكد الحجز أو اطلب تعديل التفاصيل من فريق ملامح.",
  });

  await admin.from("casting_projects").update({
    client_status_note: "تم اعتماد الاختيارات وبدأ فريق ملامح مرحلة تأكيد توفر المواهب المختارة.",
    updated_at: now,
  }).eq("id", projectId);
...
export async function markManagedBookingCompletedAction(formData: FormData) {
  ...
  const { data: booking } = await admin
    .from("talent_bookings")
    .select("id,conversation_id,status,publisher_completed_at,talent_completed_at,admin_user_id,managed_casting_project_id")
    .eq("id", bookingId)
  ...
  const { error } = await admin.from("talent_bookings").update({
    ...
    updated_at: now,
    admin_user_id: booking.admin_user_id || adminUser.id,
  }).eq("id", booking.id);
  ...
  if ((remaining ?? []).length === 0) {
    await admin.from("casting_projects").update({
      status: "completed",
      client_status_note: "اكتمل تنفيذ مشروع الكاستينغ والمواهب المؤكدة.",
      updated_at: now,
    }).eq("id", projectId);
  }
