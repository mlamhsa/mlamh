...
async function updateTalentReviewStatus({
  ...
  const { data: talent, error: talentError } = await adminClient
    .from("talents")
    .select("id, user_id, status, published")
    .eq("id", id)
    .maybeSingle();
  ...
  if (!talent.user_id) {
    return {
      success: false,
      message: locale === "ar" ? "ملف الموهبة غير مرتبط بحساب مستخدم." : "The talent profile is not linked to a user account.",
    };
  }
  ...
  if (profileError) {
    ...
    return {
      success: false,
      message: locale === "ar" ? "تعذر قراءة حالة المراجعة الحالية." : "Unable to read the current review status.",
    };
  }
  ...
  if (!profile) {
    return {
      success: false,
      message: locale === "ar" ? "لم يتم العثور على حساب الموهبة المرتبط." : "The linked talent account could not be found.",
    };
  }
  ...
  if (decision === "changes_requested" && !reason) {
    return { success: false, message: locale === "ar" ? "اكتب سبب طلب التعديل." : "Please provide a reason for requesting changes." };
  }

  if (decision === "rejected" && !reason) {
    return { success: false, message: locale === "ar" ? "اكتب سبب رفض الملف." : "Please provide a reason for rejecting the profile." };
  }
  ...
  if (profileUpdateError) {
    ...
    return {
      success: false,
      message: locale === "ar" ? "تعذر تحديث حالة مراجعة الحساب." : "Unable to update the account review status.",
    };
  }
  ...
  if (talentUpdateError) {
    ...
    await adminClient.from("profiles").update({ approval_status: previousStatus }).eq("id", profile.id);
    return {
      success: false,
      message: locale === "ar" ? "تعذر إكمال قرار المراجعة. تم إلغاء التغيير." : "Unable to complete the review decision. The change was rolled back.",
    };
  }
  ...
  if (historyError) {
    ...
    await Promise.all([
      adminClient.from("profiles").update({ approval_status: previousStatus }).eq("id", profile.id),
      adminClient.from("talents").update({ status: talent.status, published: talent.published }).eq("id", id),
    ]);
    return {
      success: false,
      message: locale === "ar" ? "تعذر حفظ سجل المراجعة، لذلك تم إلغاء القرار." : "The review history could not be saved, so the decision was rolled back.",
    };
  }
