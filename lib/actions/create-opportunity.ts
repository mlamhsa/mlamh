...
function normalizeWorkDate(
  ...
  const parsedDate = new Date(`${cleaned}T00:00:00Z`);
...
function normalizeRoleRequirements(value: unknown) {
...
export async function createOpportunityAction(
  ...
  if (
    ...
    payload.posting_mode !== "project"
  ) {
    throw new Error(
      getLocalizedMessage(safeLocale, {
        ar: "نوع الفرصة غير صالح.",
        en: "Invalid opportunity format.",
      }),
    );
  }

  const postingMode = payload.posting_mode;
  ...
  const applicationDays = numberOrNull(application_days);
  const requiredCount = numberOrNull(required_count);
  ...
  const normalizedWorkDate = normalizeWorkDate(
    work_date,
    safeLocale,
  );

  const cleanWorkTime = cleanText(work_time);
  const cleanWorkDuration = cleanText(work_duration);

  if (
    cleanWorkTime &&
    !/^([01]\d|2[0-3]):[0-5]\d$/.test(cleanWorkTime)
  ) {
    throw new Error(
      getLocalizedMessage(safeLocale, {
  ...
  const normalizedRoleRequirements =
    normalizeRoleRequirements(role_requirements);
  ...
  if (cleanWorkDuration.length > 120) {
    throw new Error(
      getLocalizedMessage(safeLocale, {
  ...
  const publisherDisplayName =
    ...
    (safeLocale === "ar" ? "ناشر على ملامح" : "MLAMH Publisher");
  ...
  const opportunity = await OpportunityService.create({
    ...
    min_age: minAge,
    max_age: maxAge,
    compensation_type: normalizedCompensationType,
    budget:
    ...
    work_time: cleanWorkTime || null,
    work_duration: cleanWorkDuration || null,
    role_requirements: normalizedRoleRequirements,
    company_name: publisherDisplayName,
  });
  ...
  try {
    await createEvent({
      ...
      metadata: {
        ...
        workTime: cleanWorkTime || null,
        workDuration: cleanWorkDuration || null,
      },
    });
  } catch (eventError) {
    console.error(
