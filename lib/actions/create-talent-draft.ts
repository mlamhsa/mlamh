...
export async function createTalentDraftAction(
  ...
  if (!selectedCategory) {
    return {
      success: false,
      message: locale === "ar" ? "اختر تخصصك للمتابعة." : "Choose your talent type to continue.",
    };
  }
  ...
  try {
    ...
    if (existingTalent) {
      ...
      if (updateError) {
        ...
        return {
          success: false,
          message: locale === "ar" ? "تعذر تحديث تخصصك." : "Unable to update your talent type.",
        };
      }
    } else {
    ...
    // Creating the talent entity is only the first saved onboarding step.
    // Do not mark onboarding as completed until required profile/photos/review steps are finished.
    ...
    if (onboardingError) {
      ...
      return {
        success: false,
        message: locale === "ar"
          ? "تم إنشاء الملف لكن تعذر حفظ تقدم التسجيل."
          : "The profile was created, but onboarding progress could not be saved.",
      };
    }
