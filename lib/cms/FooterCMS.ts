...
export class FooterCMS {
  static async getPublicFooter(
    ...
    try {
      ...
      console.error("[FooterCMS.getPublicFooter]", error);

      // Footer CMS content is non-critical. Public routes such as signup/login
      // must remain usable even when the CMS database call or its credentials
      // are temporarily unavailable in a preview environment.
      return {
        description: "",
        email: null,
        phone: null,
