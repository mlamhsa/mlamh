...
export type MobileTalentProfileUpdateInput = {
  displayName?: unknown; bio?: unknown; skills?: unknown; citySlug?: unknown; gender?: unknown; dateOfBirth?: unknown; nationalitySlug?: unknown; heightCm?: unknown; availabilityStatus?: unknown;
...
export async function updateMobileTalentProfile({ userId, locale, input }: { userId: string; locale: "ar" | "en"; input: MobileTalentProfileUpdateInput }) {
  ...
  const fields = [displayName,bio,skills,languages,dialects,modelingTypes,gender,availability,eyeColor,hairColor,hairType,skinColor,clothingSize,dateOfBirth,nationality,height,weight,shoeSize,actingAgeMin,actingAgeMax,experienceYears,readyToTravel,hasPassport,hasCar,workOutsideCity,workOutsideCountry];
  ...
  if (nationality.value !== undefined) { values.nationality_slug = nationality.value; values.nationality = nationality.value; }
