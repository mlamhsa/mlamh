...
export type MobileTalentProfileUpdateInput = {
  primaryRole?: unknown; displayName?: unknown; phone?: unknown; profileVisibility?: unknown; dataAccuracyContactConsent?: unknown; bio?: unknown; skills?: unknown; citySlug?: unknown; gender?: unknown; dateOfBirth?: unknown; nationalitySlug?: unknown; heightCm?: unknown; availabilityStatus?: unknown;
...
export async function updateMobileTalentProfile({ userId, locale, input }: { userId: string; locale: "ar" | "en"; input: MobileTalentProfileUpdateInput }) {
  ...
  const [{ data: profile, error: profileError }, { data: existingTalent, error: talentLookupError }] = await Promise.all([
    supabase
      .from("profiles")
      .select("id,approval_status,profile_completed_at,onboarding_step")
      .eq("user_id", userId)
      .maybeSingle(),
    supabase.from("talents").select("id").eq("user_id", userId).maybeSingle(),
  ]);
  if (profileError || talentLookupError) return { ok: false as const, code: "UPDATE_FAILED" as const };
  if (!profile || !existingTalent) return { ok: false as const, code: "TALENT_NOT_FOUND" as const };

  const coreFieldRequested = [
    "primaryRole", "displayName", "phone", "citySlug", "gender",
    "dateOfBirth", "nationalitySlug", "profileVisibility", "dataAccuracyContactConsent",
  ...
  if (profileVisibility.value !== undefined) values.profile_visibility = profileVisibility.value;
  ...
  if (Object.keys(values).length > 0) {
    const { error } = await supabase.from("talents").update(values).eq("id", existingTalent.id).eq("user_id", userId);
  ...
  return { ok: true as const, id: Number(existingTalent.id) };
}
