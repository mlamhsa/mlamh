...
async function resolveContext(userId: string): Promise<Context | null> {
  ...
  if (profile.account_type === "talent") {
    const { data } = await admin.from("talents").select("id,status").eq("user_id", userId).maybeSingle();
    return data && !isRestrictedAccountStatus(data.status) ? { role: "talent", entityId: data.id } : null;
  }
  if (profile.account_type === "publisher") {
    const { data } = await admin.from("publishers").select("id,status").eq("profile_id", profile.id).maybeSingle();
    return data && !isRestrictedAccountStatus(data.status) ? { role: "publisher", entityId: data.id } : null;
  }
  return null;
}
...
export async function getUserConversations(userId: string): Promise<ConversationsResponse> {
  ...
  let query = admin.from("conversations").select("id,opportunity_id,publisher_id,talent_id,conversation_type,status,updated_at").order("updated_at", { ascending: false });
  query = context.role === "talent" ? query.eq("talent_id", context.entityId) : query.eq("publisher_id", context.entityId).eq("conversation_type", "publisher_talent");
  ...
  const [messagesResult, opportunitiesResult, partiesResult] = await Promise.all([
    ...
    admin.from("opportunities").select("id,title,posting_mode").in("id", opportunityIds),
    context.role === "talent"
  ...
  const latest = new Map<number, any>(); const unread = new Map<number, number>();
  ...
  const items: MobileConversation[] = await Promise.all(rows.map(async (row) => {
    const party = parties.get(context.role === "talent" ? row.publisher_id : row.talent_id) as any;
    const last = latest.get(row.id); const opportunity = opportunities.get(row.opportunity_id) as any;
    const partyName = context.role === "talent" ? (row.conversation_type === "mlamh_talent" ? "MLAMH" : party?.company_name || party?.contact_name || "Publisher") : party?.display_name_ar || party?.display_name_en || party?.name_ar || party?.name_en || "Talent";
    const partyImageUrl = context.role === "talent" ? party?.profile_image_url ?? null : await signTalentMediaReference(party?.image_url, admin);
    return { id: row.id, opportunityId: row.opportunity_id, opportunityTitle: opportunity?.title ?? null, postingMode: opportunity?.posting_mode === "quick" ? "quick" : "casting", partyName, partyImageUrl, status: row.status, latestMessage: last?.body?.trim() || null, lastActivityAt: last?.created_at ?? row.updated_at, unreadCount: unread.get(row.id) ?? 0 };
  }));
